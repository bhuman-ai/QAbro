...
The pointer remains visible between recorded clicks at the last reached target and animates to the next target when that click occurs. Only the ripple and “Trying to click…” label are temporary. Before the first recorded click, the pointer is shown at the first known target rather than inventing raw mouse movement data.
## Decisions
...
- 2026-08-03 Cursor continuity: replay keeps one pointer visible between clicks and animates its position as recorded click targets change; ripple and intent copy remain click-only.
