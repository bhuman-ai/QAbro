...
const { validateReconRequest } = require("../../lib/submission-core");
...
module.exports = async (req, res) => {
  ...
  const validation = validateReconRequest(body);
  if (!validation.ok) {
  ...
  const queued = await enqueueSubmissionJob(request, {
    ownerUserId,
    ownerEmail,
    brandKey: sanitizeString(request.metadata?.brand_key || request.metadata?.brandKey, 256) || null,
    publicBaseUrl,
    statusUrl,
    reportUrl
  });
  if (!queued.ok) {
    return res.status(queued.status || 500).json({
      ok: false,
      error: queued.error || "Failed to enqueue submission recon job"
    });
  }
