...
module.exports = async (req, res) => {
  ...
  const result = await enqueueSubmissionPackSubmit(body, {
    ownerUserId,
    ownerEmail,
  ...
  if (!result.ok) {
    return res.status(result.status || 500).json({
      ...
      live_pack_selection: result.live_pack_selection || null
    });
  }

  return res.status(result.status || (body?.dry_run ? 200 : 202)).json({
    ok: true,
