...
test("projects handler preserves nested repo triage project metadata", async () => {
  const originalFetch = global.fetch;
  let capturedInit = null;
        ...
        return [
          {
            ...
            owner_user_id: "user_123",
            owner_email: "owner@example.com",
            last_used_at: "2026-04-01T09:00:00.000Z",
            ...
            metadata: {
              ...
              repo_triage: {
                enabled: true,
                repo: "acme/web",
                path_allowlist: ["apps/web", "packages/ui"]
              }
            }
          }
        ];
  ...
  try {
    await withEnv(
      ...
      async () => {
        ...
        assert.equal(res.statusCode, 200);
        const payload = JSON.parse(String(capturedInit.body || "[]"));
        assert.deepEqual(payload[0].metadata, {
          source: "dashboard_onboarding",
          repo_triage: {
            enabled: true,
            repo: "acme/web",
