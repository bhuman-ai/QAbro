...
async function listCanonicalQaProjects(filters = {}, options = {}) {
  ...
  const [savedProjectsResult, reportsResult] = await Promise.all([
    listQaProjects({ owner_user_id: ownerUserId }, options),
    listQaReports({ owner_user_id: ownerUserId, limit: "200", offset: "0" }, options)
  ]);

  if (!savedProjectsResult.ok) {
  ...
  const savedProjects = Array.isArray(savedProjectsResult.items) ? savedProjectsResult.items : [];
