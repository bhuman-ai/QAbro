...
function buildVisionPlannerPrompt({
  ...
  return [
    ...
    ...(forcedEmail
      ...
      : []),
    "- Include think_aloud as a short first-person user reaction, not QA jargon.",
    ...
    '  "amount": 700,',
    '  "tab_index": 0,',
    '  "think_aloud": "short first-person user thought",',
...
function buildFallbackVisionThinkAloud(decision) {
  const action = sanitizeString(decision?.action, 32).toLowerCase();
...
function buildVisionModeCandidateReport({
  ...
  } else if ((finalStatus === "failed" || finalStatus === "partial") && findings.length === 0) {
    const blockedFinding = {
      id: "finding-vision-blocked",
      type: "dead_end",
      severity: "high",
...
async function executeVisionOnlyModeAttempt({
  ...
  while (true) {
    ...
    const thinkAloud = decision.think_aloud || buildFallbackVisionThinkAloud(decision);
