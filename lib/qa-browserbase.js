...
function buildVisionPlannerPrompt({
  ...
  return [
    ...
    ...(forcedEmail
      ...
      : []),
    "- Include think_aloud as a short first-person customer reaction to the actual visible screen.",
    '- The thought must mention a visible detail, label, copy snippet, product claim, field, price, error, or page state the customer can see.',
    '- The thought should sound like a perspective customer deciding whether to trust or understand the product, not an operator describing the task.',
    '- Good think_aloud: "I can see this is about SEO clusters, but I still need to know what happens after I sign up."',
    '- Bad think_aloud: "I need to look around", "I will click the next button", "This starts the main flow", "This feels like enough to finish the task".',
    '- If the screenshot does not provide enough visible content for a real customer reaction, set think_aloud to an empty string.',
    ...
    '  "amount": 700,',
    '  "tab_index": 0,',
    '  "think_aloud": "short first-person customer reaction to visible page content",',
...
function buildFallbackVisionThinkAloud(decision) {
  const target = sanitizeString(decision?.target, 160);
  const reason = sanitizeString(decision?.reason, 220);

  if (sanitizeString(decision?.action, 32).toLowerCase() === "fail") {
...
async function executeVisionOnlyModeAttempt({
  ...
  while (true) {
    ...
    const thinkAloud = !isTaskCentricThinkAloud(decision.think_aloud)
      ? decision.think_aloud
      : buildFallbackVisionThinkAloud(decision);
    appendRunLog(runLog, "vision_only_step_decision", {
      step,
      action: decision.action,
      target: decision.target || null,
      think_aloud: thinkAloud || null,
      emotion: decision.emotion || null,
      reason: decision.reason || null
    });
    if (thinkAloud) {
      appendRunLog(runLog, "persona_thought", {
        step,
        text: thinkAloud,
        emotion: decision.emotion || null,
        action: decision.action,
