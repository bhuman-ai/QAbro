...
function extractCapturedScreenshotEntriesForPersonaRecovery(artifacts, maxItems = 4) {
  const entries = Array.isArray(artifacts?.captured_screenshot_entries) ? artifacts.captured_screenshot_entries : [];
  const normalized = entries
    .map((entry, index) => {
      const ref =
        sanitizeString(entry?.ref || entry?.data_url || entry?.dataUrl || entry?.url, 2000000) || "";
      ...
      return {
        ref,
        label: sanitizeString(entry?.label, 240) || "",
        ts: sanitizeString(entry?.ts || entry?.timestamp, 128) || null,
  ...
  const selected = [];
  const candidateIndexes = [0, Math.floor(normalized.length / 3), Math.floor((normalized.length * 2) / 3), normalized.length - 1];
  const seenIndexes = new Set();
  candidateIndexes.forEach((candidateIndex) => {
    const safeIndex = Math.max(0, Math.min(normalized.length - 1, candidateIndex));
    if (seenIndexes.has(safeIndex)) {
      return;
    }
    seenIndexes.add(safeIndex);
    selected.push(normalized[safeIndex]);
  });

  return selected.slice(0, maxItems);
}

async function regeneratePersonaReadoutFromStoredScreenshots({
  runRequest,
  artifacts,
  observationClient,
  timeoutMs,
  ...
  const selectedEntries = extractCapturedScreenshotEntriesForPersonaRecovery(artifacts, maxItems);
  if (!selectedEntries.length) {
    return { ok: false, error: "No stored screenshots are available for persona recovery." };
  }

  const visionConfig = resolveVisionOnlyConfig(runRequest, {
    visionObservationClient: observationClient,
    visionStepTimeoutMs: timeoutMs
  });
  if (!visionConfig.apiKey && typeof visionConfig.observerClient !== "function") {
    return { ok: false, error: "Vision observer is not configured." };
  }

  const personaObservations = [];
  const regeneratedRunLog = [];
  ...
  for (const [index, entry] of selectedEntries.entries()) {
    ...
    const prompt = buildVisionObservationPrompt({
      runRequest,
      step,
      currentUrl: sanitizeString(runRequest?.target_url, 4096) || "",
      historyText: "- prior browser actions unavailable during recovery",
    ...
    try {
      ...
      const observationSignature = buildVisionPersonaObservationSignature(
      ...
      const storedObservation = {
        step,
        current_url: sanitizeString(runRequest?.target_url, 4096) || "",
        label: entry.label || null,
        ...normalizedObservation
      };
      personaObservations.push(storedObservation);
      regeneratedRunLog.push({
        ts: entry.ts || new Date().toISOString(),
        event: "persona_observation",
        data: {
          step,
          current_url: storedObservation.current_url || null,
          observation: storedObservation.observation || null,
          what_i_think_this_is: storedObservation.what_i_think_this_is || null,
          noticed: storedObservation.noticed,
          skepticism: storedObservation.skepticism || null,
          missing_information: storedObservation.missing_information || null,
          trust_signal: storedObservation.trust_signal || null,
          emotion: storedObservation.emotion || null,
          continue_state: storedObservation.continue_state || null,
          regenerated: true
        }
      });
    } catch {
      // Ignore recovery failures for individual screenshots and keep trying others.
    }
  }

  const summary = summarizeVisionPersonaObservations(personaObservations);
  if (!summary) {
    return { ok: false, error: "Stored screenshots did not produce a meaningful persona readout." };
  }

  return {
    ok: true,
    summary,
    observations: personaObservations,
    runLog: regeneratedRunLog
  };
}
...
async function maybeHandleVisionManagedAuth({
  ...
  if (otpUiVisible && !state.otpSubmitted && otpBroker?.enabled && otpInbox) {
...
function buildVisionModeCandidateReport({
  ...
  const blockedCurrentState = buildVisionCurrentState(history, finalNote);
  ...
  if (
    ...
    const blockedFinding = {
      ...
      observed_behavior: finalNote || "The run stopped before completing the requested flow due to repeated action failures.",
      emotional_reaction: {
        primary: "frustration",
        intensity: 3
      },
      ...
      diagnostic_details: {
        ...
        failure_reason:
          finalNote || "The flow stayed on the same visible state and never reached the expected completed outcome.",
