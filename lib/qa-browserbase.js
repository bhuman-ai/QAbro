...
function buildVisionActionFailureTitle(decision, errorMessage) {
  const action = sanitizeString(decision?.action, 80).toLowerCase();
  const target = sanitizeString(decision?.target, 160) || "Target";
  const message = sanitizeString(errorMessage, 500).toLowerCase();
  if (action === "click") {
    if (/non_interactive_element_at_point|not interactable|not clickable|point did not match/.test(message)) {
      return `${target} was not clickable`;
    }
    return `${target} click failed`;
  }
  if (action === "type") {
...
async function clickWithVisionLocalization({
  ...
  try {
    ...
    const validateAndMaybeRetryLocalization = async () => {
    ...
    try {
      await validateAndMaybeRetryLocalization();
    ...
    try {
      ...
      const existingIssue = issues.find(
        (issue) =>
          ...
          issue?.observed_behavior === errorMessage &&
          issue?.page?.url === issuePageUrl
      );
      if (existingIssue) {
        existingIssue.type = repeatedActionFailureCount >= 2 ? "dead_end" : existingIssue.type;
        existingIssue.severity = repeatedActionFailureCount >= 2 ? "high" : existingIssue.severity;
        ...
        issues.push({
          ...
          type: repeatedActionFailureCount >= 2 ? "dead_end" : "confusion_point",
          severity: repeatedActionFailureCount >= 2 ? "high" : "medium",
          title: issueTitle,
          expected_behavior:
      ...
      if (repeatedActionFailureCount >= 3) {
        failed = true;
        outcomeType = "product_blocked";
        finalNote = `The visual agent failed the same action ${repeatedActionFailureCount} times and stopped instead of retrying indefinitely.`;
        appendRunLog(runLog, "vision_only_repeated_action_blocked", {
          step,
          repeated_action_failure_count: repeatedActionFailureCount,
          action: decision.action,
          target: decision.target || null,
