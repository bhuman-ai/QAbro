...
function buildQaTrialInviteEmailContent({ role, productName, testFocus, durationMinutes, trialUrl } = {}) {
  ...
  const heading = isTester ? "Earn your first verified QA score" : "Your free product test is ready";
  const body = isTester
  ...
  const action = isTester ? "Accept tester trial" : "Approve free test";
  const text = [
    "Before Users Do",
    "",
    heading,
...
async function sendQaTrialInviteEmail({ email, role, productName, testFocus, durationMinutes, trialUrl } = {}, options = {}) {
  const env = options.env || process.env;
  const recipients = normalizeAlertEmailList(email);
  if (!recipients.length) {
    return { ok: false, skipped: true, error: "No recipient email is configured" };
  ...
  const transporter = createQaAlertTransport(env);
  const content = buildQaTrialInviteEmailContent({ role, productName, testFocus, durationMinutes, trialUrl });
  const info = await transporter.sendMail({
    from: sanitizeString(normalizeEnvString(env.QA_ALERT_EMAIL_FROM, 320), 320),
    to: recipients,
    ...(normalizeEnvString(env.QA_ALERT_EMAIL_REPLY_TO, 320)
