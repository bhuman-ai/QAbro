...
const ALLOWED_SENDER_DOMAINS = ["beforeusersdo.com"];
...
function extractEmailAddress(value) {
  const normalized = normalizeEnvString(value, 320);
  const bracketed = normalized.match(/<([^<>]+)>/);
  const candidate = sanitizeString(bracketed?.[1] || normalized, 320).toLowerCase();
  return EMAIL_ADDRESS_PATTERN.test(candidate) ? candidate : "";
}

function isAllowedSenderAddress(value, allowedDomains) {
...
function getQaAlertEmailConfigurationError(env = process.env) {
  ...
  if (!host || !Number.isFinite(port) || port <= 0 || !username || !password || !from) {
    return "Email alerts are not configured on the server";
  }

  if (!isAllowedSenderAddress(from, ALLOWED_SENDER_DOMAINS)) {
    return `Automatic email sender must use ${ALLOWED_SENDER_DOMAINS.join(" or ")}`;
  }
  const replyTo = normalizeEnvString(env.QA_ALERT_EMAIL_REPLY_TO, 320);
  if (replyTo && !isAllowedSenderAddress(replyTo, ALLOWED_SENDER_DOMAINS)) {
    return `Automatic email reply-to must use ${ALLOWED_SENDER_DOMAINS.join(" or ")}`;
  }
  const smtpUsernameAddress = extractEmailAddress(username);
  if (smtpUsernameAddress && !isAllowedSenderAddress(smtpUsernameAddress, ALLOWED_SENDER_DOMAINS)) {
    return `Automatic email SMTP account must use ${ALLOWED_SENDER_DOMAINS.join(" or ")}`;
  }
  return "";
}

function isQaAlertEmailConfigured(env = process.env) {
  return !getQaAlertEmailConfigurationError(env);
}
...
function buildQaAlertEmailContent({ schedule = {}, alert = {}, report = {} } = {}) {
  ...
  const subject = `[Before Users Do] ${title} (${brandLabel})`.slice(0, 240);
  const text = [
    `${title}`,
    "",
    `Project: ${brandLabel}`,
  ...
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; color: #1f2937; line-height: 1.5;">
      <p style="margin: 0 0 16px; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: #6b7280;">Before Users Do alert</p>
...
function buildQaReportReadyEmailContent({ targetUrl = "", shareUrl = "", report = {} } = {}) {
  ...
  const subject = `[Before Users Do] ${reportTitle}`.slice(0, 240);
  const text = [
    `${reportTitle}`,
    "",
  ...
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; color: #1f2937; line-height: 1.6;">
      <p style="margin: 0 0 14px; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: #6b7280;">Before Users Do</p>
...
function normalizeEmailHeaderValue(value, maxLength = 240) {
  return (sanitizeOptionalString(value, maxLength) || "").replace(/[\r\n]+/g, " ").trim();
}

function normalizeMailMessageId(value) {
  const normalized = sanitizeOptionalString(value, 512) || "";
  return /[\r\n]/.test(normalized) ? "" : normalized;
}

function resolveHumanTestReportUrl(input = {}) {
  return (
    normalizeUrl(
      input.reportUrl ||
        input.report_url ||
...
function buildHumanTestReportReadyEmailContent(input = {}) {
  const brandLabel =
  ...
  const reportUrl = resolveHumanTestReportUrl(input);
  const subject = `[Before Users Do] ${brandLabel} test report is ready`.slice(0, 240);
  const text = [
    `${brandLabel} test report is ready`,
    "",
    "This report is based on the tester's recorded video and speech.",
    "",
    reportUrl ? `View your report: ${reportUrl}` : ""
  ]
    .filter(Boolean)
    .join("\n");
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; color: #10182d; line-height: 1.6; max-width: 560px; margin: 0 auto;">
      ...
      <h1 style="margin: 0 0 12px; font-size: 26px; line-height: 1.2;">${escapeHtml(brandLabel)} test report is ready</h1>
      <p style="margin: 0 0 22px; color: #5d6d89;">This report is based on the tester's recorded video and speech.</p>
      ${
        reportUrl
          ? `<a href="${escapeHtml(reportUrl)}" style="display: inline-block; padding: 12px 18px; border-radius: 10px; background: #10182d; color: #ffffff; text-decoration: none; font-weight: 800;">View your report</a>`
          : ""
      }
...
function buildQaTrialInviteEmailContent({
  ...
  const safeDuration = Math.max(10, Math.min(60, Number(durationMinutes) || 15));
...
async function sendQaAlertEmail({ schedule = {}, alert = {}, report = {} } = {}, options = {}) {
  ...
  const configurationError = getQaAlertEmailConfigurationError(env);
  if (configurationError) {
    return { ok: false, skipped: true, error: configurationError };
  }
...
async function sendHumanTestReportReadyEmail(input = {}, options = {}) {
  const env = options.env || process.env;
  const recipients = normalizeAlertEmailList(input.email);
  if (!recipients.length) {
    return { ok: false, skipped: true, error: "No recipient email is configured" };
  }
  const configurationError = getQaAlertEmailConfigurationError(env);
  ...
  if (!resolveHumanTestReportUrl(input)) {
    return { ok: false, skipped: true, error: "No valid report URL is configured" };
  }

  const transporter = createQaAlertTransport(env);
  const content = buildHumanTestReportReadyEmailContent(input);
  const messageId = normalizeMailMessageId(input.messageId || input.message_id || options.messageId);
  const info = await transporter.sendMail({
    from: sanitizeString(normalizeEnvString(env.QA_ALERT_EMAIL_FROM, 320), 320),
    to: recipients,
    ...(normalizeEnvString(env.QA_ALERT_EMAIL_REPLY_TO, 320)
  ...
  const accepted = normalizeAlertEmailList(info?.accepted || []);
  const rejected = normalizeAlertEmailList(info?.rejected || []);
  const appMessageId = sanitizeOptionalString(info?.messageId, 512) || messageId || null;
  const providerMessageId =
    sanitizeOptionalString(info?.providerMessageId || info?.provider_message_id, 512) || null;
  const providerResponse = sanitizeOptionalString(info?.response, 1200) || null;
  const providerAccepted = rejected.length === 0 && accepted.length > 0;
  ...
  return {
    ...
    accepted,
    rejected,
    ...(providerAccepted ? {} : { error: "The email provider did not accept the recipient" })
  };
}
