...
function buildTesterJobAvailableEmailContent({ name, durationMinutes, jobsUrl } = {}) {
  const safeName = sanitizeOptionalString(name, 120);
  const safeDuration = Math.max(10, Math.min(60, Number(durationMinutes) || 30));
  const safeJobsUrl = normalizeUrl(jobsUrl) || "";
  const subject = "A tester qualification is ready";
  ...
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; color: #10182d; line-height: 1.6; max-width: 560px; margin: 0 auto;">
      ...
      <h1 style="margin: 0 0 12px; font-size: 26px; line-height: 1.2;">A tester qualification is ready</h1>
      <p style="margin: 0 0 12px; color: #33415c;">A new ${safeDuration}-minute qualification is ready.</p>
      <p style="margin: 0 0 22px; color: #5d6d89;">It is unpaid and creates your first verified tester score. Use Chrome on a computer. One tester can claim it, so it is first come, first served.</p>
      ${
        safeJobsUrl
...
async function sendTesterJobAvailableEmail(
  { email, name, durationMinutes, jobsUrl } = {},
  options = {}
) {
  const env = options.env || process.env;
  const recipients = normalizeAlertEmailList(email);
  ...
  const transporter = createQaAlertTransport(env);
  const content = buildTesterJobAvailableEmailContent({ name, durationMinutes, jobsUrl });
  const info = await transporter.sendMail({
    from: sanitizeString(normalizeEnvString(env.QA_ALERT_EMAIL_FROM, 320), 320),
    to: recipients,
    ...(normalizeEnvString(env.QA_ALERT_EMAIL_REPLY_TO, 320)
