...
const QA_MODE_SET = new Set(["live_qa", "controlled_ux"]);
...
function normalizeExecutionEngine(value, fallbackValue = DEFAULT_EXECUTION_ENGINE) {
  ...
  if (["browserbase", "browserbase_agent", "browserbase-agent", "stagehand"].includes(normalized)) {
...
function normalizeQaMode(value) {
  const normalized = sanitizeString(value, 64).toLowerCase().replace(/[\s-]+/g, "_");
  if (
    [
      "controlled",
      "ux_validation",
    ...
    return "controlled_ux";
  }
  return QA_MODE_SET.has(normalized) ? normalized : "live_qa";
}

function readQaMode(runRequest) {
  const metadata = isPlainObject(runRequest?.metadata) ? runRequest.metadata : {};
  return normalizeQaMode(
    metadata.qa_mode ||
      metadata.qaMode ||
      metadata.run_mode ||
      metadata.runMode ||
...
function readControlledUxConfig(runRequest) {
  const metadata = isPlainObject(runRequest?.metadata) ? runRequest.metadata : {};
  const controlled = isPlainObject(metadata.controlled_ux)
    ? metadata.controlled_ux
    : isPlainObject(metadata.controlledUx)
      ? metadata.controlledUx
      : {};

  return {
    enabled: readQaMode(runRequest) === "controlled_ux" || controlled.enabled === true,
    user_job: sanitizeOptionalString(controlled.user_job || controlled.userJob, 1000) || null,
    entry_path: sanitizeOptionalString(controlled.entry_path || controlled.entryPath, 240) || null,
    route_hints: sanitizeStringList(controlled.route_hints || controlled.routeHints, 12, 240),
    success_signals: sanitizeStringList(controlled.success_signals || controlled.successSignals, 12, 240)
  };
}

function readValidationTarget(runRequest) {
  const metadata = isPlainObject(runRequest?.metadata) ? runRequest.metadata : {};
  const raw = sanitizeString(metadata.validation_target || metadata.validationTarget, 64)
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  if (raw === "login_signup" || raw === "auth" || raw === "login") {
    return "login_signup";
  }
  if (raw === "inside_product" || raw === "authenticated" || raw === "post_login") {
    return "inside_product";
  }
  return "public_flow";
}

function readAccessMethod(runRequest, validationTarget = readValidationTarget(runRequest)) {
  const metadata = isPlainObject(runRequest?.metadata) ? runRequest.metadata : {};
  const raw = sanitizeString(metadata.access_method || metadata.accessMethod, 64)
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  if (validationTarget === "public_flow") {
    return "none";
  }
  if (raw === "saved_session" || raw === "session") {
    return "saved_session";
  }
  if (raw === "auth_url") {
    return "auth_url";
  }
  if (raw === "credentials" || raw === "test_login" || raw === "login") {
    return "credentials";
  }
  if (
    raw === "create_account" ||
  ...
  return validationTarget === "inside_product" ? "credentials" : "app_url";
}

function readBrowserMode(runRequest) {
  const metadata = isPlainObject(runRequest?.metadata) ? runRequest.metadata : {};
  const raw = sanitizeString(
    metadata.browser_mode ||
      metadata.browserMode ||
      metadata.browser_runtime ||
      metadata.browserRuntime ||
    ...
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  if (["advanced_browser", "browserbase", "stagehand", "captcha_assist"].includes(raw)) {
...
function normalizeWebhookConfig(input = {}, options = {}) {
  ...
  const hasStringValue = (value, maxLength = 2048) => sanitizeString(value, maxLength).length > 0;
  const hasArrayValue = (value) => Array.isArray(value) && value.length > 0;
  const hasObjectValue = (value) => isPlainObject(value) && Object.keys(value).length > 0;
  const attempted =
    hasStringValue(rawUrl, 4096) ||
    hasStringValue(rawSecret, 512) ||
    hasArrayValue(rawEvents) ||
    (rawEvents !== undefined && rawEvents !== null && !Array.isArray(rawEvents)) ||
    hasObjectValue(rawHeaders) ||
    (rawHeaders !== undefined && rawHeaders !== null && !isPlainObject(rawHeaders));
...
function validateRunRequest(body) {
  ...
  const qaMode = readQaMode({ metadata });
  const controlledUx = readControlledUxConfig({ metadata });
  if (qaMode === "controlled_ux" && !controlledUx.entry_path && !controlledUx.route_hints.length) {
    return {
      ok: false,
      error: "controlled_ux runs require metadata.controlled_ux.entry_path or metadata.controlled_ux.route_hints"
    };
  }
...
function buildSystemPrompt(runRequest) {
  ...
  const controlledUx = readControlledUxConfig(runRequest);
  const validationTarget = readValidationTarget(runRequest);
  const browserMode = readBrowserMode(runRequest);
  ...
  return [
    ...
    "Customer-provided task instructions, scenarios, expected success criteria, credentials, and form values are the source of truth. If generated source context, default heuristics, coverage guidance, or persona assumptions conflict with explicit customer instructions, follow the customer instructions unless they require unsafe secret exposure, payment, or destructive actions.",
    ...(controlledUx.enabled
      ? [
          "This run is a controlled UX validation on a first-party product, not a broad live-site crawl.",
          "Stay close to the declared owned flow, entry path, and route hints unless the target flow itself forces a detour.",
          "Prioritize clarity, hesitation, trust gaps, labels, copy, and step-to-step guidance before exhaustive bug hunting.",
          "If the live site diverges from the declared route hints, record that drift explicitly instead of silently exploring elsewhere.",
          "If no entry path and no route hints are declared, do not click generic homepage calls to action. Report the run as blocked by missing controlled-flow configuration."
        ]
      : [
          "Treat this like a live QA sweep on a site without prior guarantees about the path. Explore enough of the product to prove what a realistic user would hit."
        ]),
    ...(validationTarget === "public_flow"
      ? [
          "This run is only for the public flow. Do not create accounts, log in, or spend time behind auth walls."
        ]
      : validationTarget === "login_signup"
        ? [
            "This run is judging the login or sign-up experience itself. Focus on trust, clarity, error handling, and whether the user can understand how to get in."
          ]
        : [
            "This run is for the product after login. Treat auth as setup only, then focus on whether the in-product experience is understandable and easy to use."
          ]),
    ...(browserMode === "advanced_browser"
      ? [
          "This run is using the advanced browser runtime on the DO worker. Prefer the fresh local browser session and supported captcha-solving paths when the site resists normal automation."
        ]
      : []),
    ...
    "Run a mandatory copy-quality pass on every high-salience surface you see: headlines, subheads, primary calls to action, pricing claims, form guidance, empty states, and error messages.",
    "Flag copy that would immediately feel weak or machine-written to a normal visitor: vague buzzwords, generic formulaic claims, robotic or repetitive sentence patterns, needless over-explaining, unsupported certainty or social proof, awkward tone, and copy that says a lot without explaining the concrete product or outcome.",
    "Do not claim that AI wrote the copy because authorship cannot be proven from the page. Describe the observable problem instead, such as generic, templated, robotic, vague, repetitive, or untrustworthy.",
    "For every copy_issue, quote the exact visible words in observed_behavior and element.text, explain the likely visitor reaction and business impact, and attach the screenshot showing that copy. Do not file a copy issue based only on personal style preference.",
    "Run a mandatory visual-quality pass on every important screen. Judge hierarchy, information density, spacing rhythm, alignment, typography, contrast, composition, affordance, responsive fit, empty/loading/error states, brand fit, and overall professional polish.",
    "Explicitly apply the AI-slop test: if a normal visitor would immediately believe the interface came from a generic AI template, identify the visible tells. Common tells include interchangeable card grids, cards nested inside cards, excessive rounded rectangles and generic shadows, purple-blue gradients, cyan-on-dark glow, decorative glassmorphism, gradient text, icon-above-heading repetition, centered-everything layouts, weak hierarchy, and identical spacing everywhere.",
    "Do not file a visual issue merely because the design is minimal, conventional, or not to your personal taste. Tie every visual_quality_issue to a specific visible pattern, the user's likely reaction or task impact, and screenshot evidence.",
    ...
    "An automation miss is not a product bug. If the tester cannot prove that its click or input reached the intended control, classify the result as test_inconclusive, never bug or dead_end.",
    "Only report a product interaction bug after the intended target was verified and the captured post-action evidence shows that the product still failed to respond.",
    ...
    `Use the selected scope mode to prioritize coverage: ${scope.label} (${scope.time_budget_minutes} minute limit, up to ${scope.max_steps} steps). Work toward the objective until complete or blocked; the runner enforces these safety limits.`,
...
function buildTaskPrompt(runRequest) {
  ...
  const controlledUx = readControlledUxConfig(runRequest);
  const validationTarget = readValidationTarget(runRequest);
  const accessMethod = readAccessMethod(runRequest, validationTarget);
  const browserMode = readBrowserMode(runRequest);
  const authEntryUrl = sanitizeString(metadata.auth_entry_url || metadata.authEntryUrl, 4096) || "";
  ...
  const controlledModeText = controlledUx.enabled
    ? [
        "Run mode: Controlled UX validation",
        `Owned flow entry path: ${controlledUx.entry_path || "Not provided"}`,
        `Owned flow job: ${controlledUx.user_job || primaryGoal}`,
        "Planned route hints:",
        controlledUx.route_hints.length ? controlledUx.route_hints.map((item) => `- ${item}`).join("\n") : "- none provided",
        "Success checks:",
        controlledUx.success_signals.length
          ? controlledUx.success_signals.map((item) => `- ${item}`).join("\n")
          : "- none provided",
        "Controlled-mode rules:",
        "- Stay on the owned flow and nearby supporting pages unless the flow breaks.",
        "- Treat route hints as guidance from the product team, not proof. Confirm them on the live site.",
        "- Focus on whether the UI is understandable, trustworthy, and easy to move through.",
        "- If both entry path and route hints are missing, stop and report missing controlled-flow setup instead of clicking generic homepage CTAs.",
        "- Do not spend the run on unrelated exploratory coverage."
      ].join("\n")
    : "Run mode: Live QA exploration";
  ...
  const validationTargetText =
    validationTarget === "login_signup"
      ? "Validation target: Login or sign-up"
      : validationTarget === "inside_product"
        ? "Validation target: Inside the product"
        : "Validation target: Public flow";
  const accessMethodText =
    validationTarget === "public_flow"
      ? "Access method: No login needed"
      : accessMethod === "saved_session"
        ? "Access method: Use saved project session"
      : accessMethod === "create_account"
        ? `Access method: Create a fresh account during the run${authEntryUrl ? ` via ${authEntryUrl}` : ""}`
      : accessMethod === "auth_url"
        ? `Access method: Start from a specific auth URL (${authEntryUrl || "not provided"})`
        : accessMethod === "credentials"
          ? `Access method: Use test login${authEntryUrl ? ` via ${authEntryUrl}` : ""}`
          : "Access method: Start from the app URL";
  const browserModeText =
    browserMode === "advanced_browser"
      ? "Browser runtime: Advanced browser (DO worker local browser with supported captcha handling)"
      : "Browser runtime: Standard browser";
  const validationRules =
    validationTarget === "public_flow"
      ? [
          "Public-flow rules:",
          "- Stay on public pages and public CTAs.",
          "- Do not create accounts or log in.",
          "- If a wall blocks progress, note the boundary and continue with other public surfaces."
        ]
      : validationTarget === "login_signup"
        ? [
            "Auth-flow rules:",
            "- Judge the login or sign-up experience itself, not just whether a session eventually exists.",
            "- Focus on copy, trust, field labels, errors, and what happens right after submit.",
            "- Do not bypass auth in this mode."
          ]
        : [
            "Inside-product rules:",
            accessMethod === "saved_session"
              ? "- Reuse the saved project session when it is still valid."
              : "- Use the provided test login to get into the product quickly.",
            "- Once authenticated, spend the run on the in-product workflow rather than public pages.",
            "- If access setup fails, report that clearly instead of wandering elsewhere."
          ];
  ...
  return [
    ...
    "Instruction priority: follow explicit customer task instructions, scenario steps, expected success criteria, credentials, and form values before generated implementation context, default heuristics, or coverage checklist items. Use internal guidance only as fallback unless the customer instruction asks for unsafe secret exposure, payment, or destructive actions.",
    "Coverage checklist:",
    ...
    "3. Copy-quality pass: inspect prominent visible copy for vague buzzwords, formulaic or robotic phrasing, repetition, unsupported claims, tone mismatch, and over-explaining. Judge the visitor impact, not whether AI actually authored it.",
    "4. Visual-quality pass: inspect hierarchy, clutter, spacing, alignment, typography, contrast, generic AI-template patterns, responsive fit, interaction affordance, and unfinished states. Judge concrete user and trust impact, not personal taste.",
    "5. Friction and confusion probing: forms, empty states, validation, unclear copy, dead ends.",
    "6. Bug and stability pass: broken UI, failed transitions, client errors, inconsistent state.",
    "7. Aha moment capture: note any unexpectedly clear or trust-building moment.",
    "8. Goal verification: confirm whether the persona actually completed or was blocked from the intended job to be done.",
    "9. Report synthesis: create strict JSON then Markdown with evidence links.",
    ...
    "- If an automated click or input could not be verified against the intended target, use test_inconclusive. Never turn uncertain coordinates, localization failure, or runner inability into bug or dead_end.",
    "- A product interaction bug requires both target_hit_verified=true and post-action evidence showing the product failed after the verified interaction.",
    ...
    "- Classify materially weak, generic, robotic, vague, repetitive, or untrustworthy visible wording as copy_issue. Quote the exact words in observed_behavior and element.text, explain why a visitor would notice or distrust them, and show the copy in the finding screenshot.",
    "- Never state that copy was AI-generated as a fact. Report only the visible qualities and their likely effect on a real visitor.",
    "- Classify materially poor hierarchy, clutter, spacing, alignment, typography, contrast, composition, affordance, responsiveness, state design, or generic AI-template styling as visual_quality_issue. Name the exact visible pattern, explain the user or trust impact, and attach the screenshot that proves it.",
    "- For AI-template concerns, list the concrete visual tells instead of merely saying the UI looks AI-generated.",
    ...
    "- Use externally accessible asset URLs when available.",
...
function normalizeFindingType(value) {
  ...
  const aliases = {
    ...
    copy: "copy_issue",
    visual: "visual_quality_issue",
    visualquality: "visual_quality_issue",
    designquality: "visual_quality_issue",
    inconclusive: "test_inconclusive",
    testinconclusive: "test_inconclusive"
  };
...
function formatNetworkTimelineText(entry) {
  ...
  const status =
    rawStatus !== null && rawStatus !== undefined && rawStatus !== "" && Number.isFinite(numericStatus) && numericStatus > 0
      ? String(Math.round(numericStatus))
      : "";
...
function selectRelevantTimelineEntries(entries, finding, options = {}) {
  ...
  if (importanceFilter) {
    const importantSlice = important.slice(important.length - maxItems);
    return uniqueStringList(
      importantSlice.map((entry) => sanitizeString(entry?.text, 1800)).filter(Boolean),
      maxItems,
      1800
    );
  }
...
function isExperienceBlockerText(value) {
  ...
  return /timed out|timeout|stalled|same waiting state|same state|never advanced|did not advance|did not progress|blocked|could not|failed|error|submit button|not progress|spinner|blank white screen|blank page|blank screen|empty white screen|nothing is visible|no ui elements visible/i.test(message);
}
...
function buildExperienceSpanLogs(entries, startMs, endMs, maxItems = 4, importanceFilter = null) {
  ...
  if (typeof importanceFilter === "function") {
    return uniqueStringList(
      selected
        .slice(-maxItems)
...
function buildExperienceTimeline(findings, context = {}, evidenceGallery = null) {
  ...
  const spans = mergeAdjacentExperienceTimelineSpans(
    candidateFindings.flatMap((finding) =>
      buildExperienceTimelineSpansForFinding(finding, baseAbsoluteMs, consoleEntries, networkEntries)
    )
  );
  if (!spans.length) {
    return null;
...
function readReplayInteractionCoordinates(value) {
  ...
  const box = isPlainObject(payload.box) ? payload.box : {};
  const boxX = Number(box.center_x);
  const boxY = Number(box.center_y);
  return Number.isFinite(boxX) && Number.isFinite(boxY) ? { x: boxX, y: boxY } : null;
}

function normalizeReplayInteractions(value, runLog = []) {
  const interactions = [];
  const seen = new Set();
  const pushInteraction = (raw, index, fallbackTimestamp = "") => {
    if (!isPlainObject(raw)) {
      return;
    }
    const coordinates = readReplayInteractionCoordinates(raw);
    const timestampValue = sanitizeString(raw.ts || raw.timestamp || fallbackTimestamp, 128);
    const timestampMs = parseTimestampMs(timestampValue);
    if (!coordinates || !Number.isFinite(timestampMs)) {
      return;
    }
    const target = sanitizeString(raw.target || raw.describe || raw.label, 280) || "click target";
    const key = `${timestampMs}:${Math.round(coordinates.x)}:${Math.round(coordinates.y)}:${target.toLowerCase()}`;
    if (seen.has(key)) {
      return;
    }
    seen.add(key);
    interactions.push({
      id: sanitizeString(raw.id, 128) || `click-${index + 1}`,
      kind: "click",
      ts: new Date(timestampMs).toISOString(),
      target,
      x: Math.max(0, Math.round(coordinates.x * 100) / 100),
      y: Math.max(0, Math.round(coordinates.y * 100) / 100),
      source: sanitizeString(raw.source, 64) || "ai_browser"
    });
  };

  (Array.isArray(value) ? value : []).forEach((item, index) => pushInteraction(item, index));

  const safeRunLog = Array.isArray(runLog) ? runLog : [];
  const hasExplicitAttempts = safeRunLog.some((entry) =>
    sanitizeString(entry?.event, 128).toLowerCase() === "agent_click_attempted"
  );
  safeRunLog.forEach((entry, index) => {
    ...
    const isHistoricalFallback = !hasExplicitAttempts && event === "agent_click_coordinate_fallback_succeeded";
    if (event !== "agent_click_attempted" && !isHistoricalFallback) {
      return;
    }
    const payload = isPlainObject(entry.data)
      ? entry.data
      : isPlainObject(entry.details)
        ? entry.details
        : {};
    pushInteraction(payload, interactions.length + index, entry.ts || entry.timestamp);
  });
...
function normalizeInteractionVerification(value) {
  if (!isPlainObject(value)) {
    return null;
  }
  const targetHitVerified = readField(value, ["target_hit_verified", "targetHitVerified"]) === true;
  return {
    status:
      sanitizeOptionalString(readField(value, ["status"]), 64) ||
      (targetHitVerified ? "verified" : "unverified"),
    target_hit_verified: targetHitVerified,
    ...(sanitizeOptionalString(readField(value, ["target"]), 240)
      ? { target: sanitizeOptionalString(readField(value, ["target"]), 240) }
      : {}),
    ...(sanitizeOptionalString(readField(value, ["reason"]), 1000)
      ? { reason: sanitizeOptionalString(readField(value, ["reason"]), 1000) }
      : {}),
    ...(sanitizeOptionalString(readField(value, ["strategy"]), 120)
      ? { strategy: sanitizeOptionalString(readField(value, ["strategy"]), 120) }
      : {})
  };
}
...
function normalizeFailureDiagnostics(value, context = {}) {
  ...
  const repeatedActionFailureCount = Number(
    readField(rawDetails, ["repeated_action_failure_count", "repeatedActionFailureCount"])
  );
  const interactionVerification = normalizeInteractionVerification(
    readField(rawDetails, ["interaction_verification", "interactionVerification"])
  );
  ...
  return {
    ...
    ...(Number.isFinite(repeatedStateCount) && repeatedStateCount > 0
      ...
      : {}),
    ...(Number.isFinite(repeatedActionFailureCount) && repeatedActionFailureCount > 0
      ? { repeated_action_failure_count: Math.round(repeatedActionFailureCount) }
      : {}),
    ...(interactionVerification ? { interaction_verification: interactionVerification } : {})
  };
}
...
function normalizeDiagnosticDetails(value, context = {}) {
  ...
  const bodyTextLength = Number(readField(rawDetails, ["body_text_length", "bodyTextLength"]));
  const pageErrorCount = Number(readField(rawDetails, ["page_error_count", "pageErrorCount"]));
  const consoleErrorCount = Number(readField(rawDetails, ["console_error_count", "consoleErrorCount"]));
  const failedRequestCount = Number(readField(rawDetails, ["failed_request_count", "failedRequestCount"]));
  ...
  return {
    ...
    ...(sanitizeOptionalString(readField(rawDetails, ["browser_version", "browserVersion"]), 120)
      ? { browser_version: sanitizeOptionalString(readField(rawDetails, ["browser_version", "browserVersion"]), 120) }
      : {}),
    ...(isPlainObject(readField(rawDetails, ["viewport"]))
      ? { viewport: readField(rawDetails, ["viewport"]) }
      : {}),
    ...(sanitizeOptionalString(readField(rawDetails, ["document_ready_state", "documentReadyState"]), 80)
      ? {
          document_ready_state: sanitizeOptionalString(
            readField(rawDetails, ["document_ready_state", "documentReadyState"]),
            80
          )
        }
      : {}),
    ...(Number.isFinite(bodyTextLength) && bodyTextLength >= 0
      ? { body_text_length: Math.round(bodyTextLength) }
      : {}),
    ...(sanitizeOptionalString(readField(rawDetails, ["visible_text_preview", "visibleTextPreview"]), 1200)
      ? {
          visible_text_preview: sanitizeOptionalString(
            readField(rawDetails, ["visible_text_preview", "visibleTextPreview"]),
            1200
          )
        }
      : {}),
    ...(isPlainObject(readField(rawDetails, ["final_screenshot_visual_summary", "finalScreenshotVisualSummary"]))
      ? {
          final_screenshot_visual_summary: readField(rawDetails, [
            "final_screenshot_visual_summary",
            "finalScreenshotVisualSummary"
          ])
        }
      : {}),
    ...(Array.isArray(readField(rawDetails, ["dom_snapshot", "domSnapshot"]))
      ? { dom_snapshot: readField(rawDetails, ["dom_snapshot", "domSnapshot"]).slice(0, 80) }
      : {}),
    ...(readField(rawDetails, ["accessibility_snapshot", "accessibilitySnapshot"])
      ? { accessibility_snapshot: readField(rawDetails, ["accessibility_snapshot", "accessibilitySnapshot"]) }
      : {}),
    ...(Array.isArray(readField(rawDetails, ["asset_fingerprints", "assetFingerprints"]))
      ? { asset_fingerprints: readField(rawDetails, ["asset_fingerprints", "assetFingerprints"]).slice(0, 30) }
      : {}),
    ...(isPlainObject(readField(rawDetails, ["post_auth_state", "postAuthState"]))
      ? { post_auth_state: readField(rawDetails, ["post_auth_state", "postAuthState"]) }
      : {}),
    ...(Number.isFinite(pageErrorCount) && pageErrorCount >= 0
      ? { page_error_count: Math.round(pageErrorCount) }
      : {}),
    ...(Number.isFinite(consoleErrorCount) && consoleErrorCount >= 0
      ? { console_error_count: Math.round(consoleErrorCount) }
      : {}),
    ...(Number.isFinite(failedRequestCount) && failedRequestCount >= 0
      ? { failed_request_count: Math.round(failedRequestCount) }
      : {}),
    ...(Array.isArray(readField(rawDetails, ["page_errors", "pageErrors"]))
      ? { page_errors: readField(rawDetails, ["page_errors", "pageErrors"]).slice(0, 8) }
      : {}),
    ...(Array.isArray(readField(rawDetails, ["console_errors", "consoleErrors"]))
      ? { console_errors: readField(rawDetails, ["console_errors", "consoleErrors"]).slice(0, 8) }
      : {}),
    ...(Array.isArray(readField(rawDetails, ["failed_requests", "failedRequests"]))
      ? { failed_requests: readField(rawDetails, ["failed_requests", "failedRequests"]).slice(0, 12) }
      : {}),
    ...(Number.isFinite(repeatedStateCount) && repeatedStateCount > 0
      ? { repeated_state_count: Math.round(repeatedStateCount) }
      : {}),
...
function isPostOtpBlankScreenFinding(finding) {
  const diagnostics = isPlainObject(finding?.diagnostic_details) ? finding.diagnostic_details : {};
  const attemptedActions = Array.isArray(diagnostics.attempted_actions) ? diagnostics.attempted_actions : [];
  const combinedText = sanitizeString(
    [
      finding?.title,
      finding?.observed_behavior,
      diagnostics.failure_reason,
      diagnostics.current_state,
      ...attemptedActions.flatMap((attempt) => [attempt?.target, attempt?.outcome, attempt?.note])
    ]
      .filter(Boolean)
      .join(" "),
    8000
  ...
  const sawOtp =
    /(otp|verification code|security code)/.test(combinedText) &&
    /(filled|entered|submitted|submission|after filling|after successful|successful)/.test(combinedText);
  const sawPostOtpSubmit = attemptedActions.some((attempt, index) => {
    const action = sanitizeString(attempt?.action, 80).toLowerCase();
    const target = sanitizeString(attempt?.target, 500).toLowerCase();
    const outcome = sanitizeString(attempt?.outcome, 1000).toLowerCase();
    if (!(action === "press" || action === "click" || action === "submit")) {
      return false;
    }
    if (!/(enter|continue|verify|send|submit|code)/.test(target) && !/(after filling otp|verification|otp|code)/.test(outcome)) {
      return false;
    }
    return attemptedActions.slice(0, index).some((previous) =>
      /(otp|verification code|security code)/i.test(
        [previous?.target, previous?.outcome, previous?.note].filter(Boolean).join(" ")
      )
    );
  });
  const sawBlank =
    /blank white screen|blank page|blank screen|page is blank|empty white screen|screen is entirely white|nothing is visible|no ui elements visible|no loading indicator|zero information/.test(
      combinedText
    );
  const sawWaitStall = /same waiting state|never advanced|did not advance|did not progress|page to fully load|stayed visible|waiting state/.test(
    combinedText
  );

  return (sawOtp || sawPostOtpSubmit) && (sawBlank || sawWaitStall);
}
...
function deriveSpecificFindingTitle(finding, fallbackTitle = "") {
  ...
  if (isPostOtpBlankScreenFinding(finding)) {
    return "Blank white screen after successful OTP verification";
  }
...
function normalizeFinding(finding, index, context = {}) {
  ...
  const interactionVerification = normalized.diagnostic_details?.interaction_verification;
  const interactionWasUnverified =
    isPlainObject(interactionVerification) &&
    (interactionVerification.target_hit_verified === false ||
      ["unverified", "uncertain", "unavailable", "failed"].includes(
        sanitizeString(interactionVerification.status, 64).toLowerCase()
      ));
  if (
    interactionWasUnverified &&
    ["bug", "dead_end", "frustration_point", "confusion_point"].includes(normalized.type)
  ) {
    const attemptedActions = Array.isArray(normalized.diagnostic_details.attempted_actions)
      ? normalized.diagnostic_details.attempted_actions
      : [];
    const target =
      sanitizeString(interactionVerification.target, 240) ||
      sanitizeString([...attemptedActions].reverse().find((attempt) => sanitizeString(attempt?.target, 500))?.target, 240) ||
      sanitizeString(normalized.element?.text, 240) ||
      "the intended control";
    const reason =
      sanitizeString(interactionVerification.reason, 1000) ||
      "The automated tester could not prove that its interaction reached the intended target.";
    normalized.type = "test_inconclusive";
    normalized.severity = "medium";
    normalized.title = `Tester could not verify ${target}`.slice(0, 180);
    ...
    normalized.observed_behavior =
      `The tester could not verify that it interacted with "${target}". ${reason} This is not proof of a product bug.`;
    normalized.emotional_reaction = { primary: "uncertainty", intensity: 2 };
    normalized.fix_hint = "Do not change the product based on this evidence. Re-run the test or inspect this step manually.";
    normalized.confidence = 0.98;
    normalized.tags = uniqueStringList([...(normalized.tags || []), "automation-uncertainty"], 10, 120);
  }
...
function normalizeJourneyStepClips(value) {
  ...
  for (const rawClip of source.slice(0, 24)) {
    ...
    clips.push({
      ...
      ...(sanitizeString(rawClip.finding_id || rawClip.findingId, 128)
        ? { finding_id: sanitizeString(rawClip.finding_id || rawClip.findingId, 128) }
        : {}),
      ...(sanitizeString(rawClip.finding_title || rawClip.findingTitle, 240)
        ? { finding_title: sanitizeString(rawClip.finding_title || rawClip.findingTitle, 240) }
        : {}),
      ...(sanitizeString(rawClip.finding_type || rawClip.findingType, 64)
        ? { finding_type: sanitizeString(rawClip.finding_type || rawClip.findingType, 64) }
        : {}),
      ...(sanitizeString(rawClip.level, 32)
        ? { level: sanitizeString(rawClip.level, 32).toLowerCase() }
        : {})
    });
  }
...
function enrichBlockedFindingFromJourneys(finding, journeys, context = {}) {
  ...
  const findingObservation = simplifyHumanNarrative(finding.observed_behavior, 2000);
  const journeyObservation =
  ...
  const observation = isExperienceBlockerText(findingObservation)
    ? findingObservation
    : journeyObservation || findingObservation;
...
function buildTopIssues(findings) {
  ...
  return findings
    .filter((finding) => finding.type !== "test_inconclusive")
    .slice()
    .sort((a, b) => {
...
function isGenericSyntheticDeadEndFinding(finding) {
  const id = sanitizeString(finding?.id, 128).toLowerCase();
  const title = sanitizeString(finding?.title, 220).toLowerCase();
  if (id === "finding_dead_end_1" || id === "finding-vision-blocked") {
    return true;
  }
  return (
...
function hasFindingBlockerEvidence(finding) {
  const diagnostics = isPlainObject(finding?.diagnostic_details) ? finding.diagnostic_details : {};
  const message = [
    finding?.observed_behavior,
    diagnostics.failure_reason,
    ...
    .join(" ");
  return (
    isExperienceBlockerText(message) ||
    isEarlyNavigationFailureMessage(message) ||
    isAuthSetupFailureMessage(message)
  );
}
...
function normalizeReport(options = {}) {
  ...
  const candidateSummaryNote = sanitizeOptionalString(candidateSummary.note || candidateSummary.notes, 2000);
  const summaryLooksLikeFailure =
    Boolean(candidateSummaryNote) &&
    (isEarlyNavigationFailureMessage(candidateSummaryNote) ||
      isAuthSetupFailureMessage(candidateSummaryNote) ||
      (status !== "completed" && isExperienceBlockerText(candidateSummaryNote)));
  const initialFailureMessage =
    sanitizeOptionalString(options.failureMessage, 2000) || (summaryLooksLikeFailure ? candidateSummaryNote : "");
  ...
  findings = findings.filter((finding) => {
    if (status !== "partial" || summaryLooksLikeFailure || !isGenericSyntheticDeadEndFinding(finding)) {
      return true;
    }
    return hasFindingBlockerEvidence(finding);
  });
  ...
  const personaOverall =
    sanitizeOptionalString(
      candidateSummary.persona_overall ||
        candidateSummary.persona_readout ||
        candidateSummary.user_reaction ||
        candidateSummary.persona_reaction,
      1600
    ) || null;
  const personaEmotionalState =
    sanitizeOptionalString(
      candidateSummary.emotional_state || candidateSummary.persona_emotional_state,
      600
    ) || null;
  const personaTakeaways = sanitizeStringList(
    candidateSummary.persona_takeaways || candidateSummary.takeaways || candidateSummary.user_takeaways,
    6,
    240
  );
  const personaSkepticisms = sanitizeStringList(
    candidateSummary.persona_skepticisms ||
      candidateSummary.skepticisms ||
      candidateSummary.trust_concerns ||
      candidateSummary.user_skepticisms,
    6,
    240
  );
  const replayInteractions = normalizeReplayInteractions(
    readField(candidateReport, ["replay_interactions", "replayInteractions"]),
    context.runLog
  );
  ...
  const normalized = {
    ...
    summary: {
      counts,
      risk_score:
        typeof candidateSummary.risk_score === "number" &&
        Number.isFinite(candidateSummary.risk_score) &&
        !findings.some((finding) => finding.type === "test_inconclusive")
    ...
    ...(replayInteractions.length ? { replay_interactions: replayInteractions } : {}),
    recommendations,
...
function buildMarkdownReport(report, runRequest, extras = {}) {
  ...
  for (const finding of findings) {
    ...
    if (diagnosticDetails.browser_version) {
      lines.push(`- Browser version: ${diagnosticDetails.browser_version}`);
    }
    if (diagnosticDetails.viewport && typeof diagnosticDetails.viewport === "object") {
      lines.push(
        `- Viewport: ${diagnosticDetails.viewport.width || "?"}x${diagnosticDetails.viewport.height || "?"}`
      );
    }
    if (diagnosticDetails.document_ready_state || Number.isFinite(Number(diagnosticDetails.body_text_length))) {
      lines.push(
        `- Final document: readyState=${diagnosticDetails.document_ready_state || "n/a"}, bodyTextLength=${
          Number.isFinite(Number(diagnosticDetails.body_text_length)) ? Math.round(Number(diagnosticDetails.body_text_length)) : "n/a"
        }`
      );
    }
    if (diagnosticDetails.final_screenshot_visual_summary && typeof diagnosticDetails.final_screenshot_visual_summary === "object") {
      const visualSummary = diagnosticDetails.final_screenshot_visual_summary;
      lines.push(
        `- Final screenshot visual state: looks_blank=${visualSummary.looks_blank === true ? "true" : "false"}, mostly_white_ratio=${
          Number.isFinite(Number(visualSummary.mostly_white_ratio)) ? Number(visualSummary.mostly_white_ratio) : "n/a"
        }, non_white_ratio=${
          Number.isFinite(Number(visualSummary.non_white_ratio)) ? Number(visualSummary.non_white_ratio) : "n/a"
        }`
      );
    }
    if (diagnosticDetails.post_auth_state && typeof diagnosticDetails.post_auth_state === "object") {
      const postAuthState = diagnosticDetails.post_auth_state;
      lines.push(
        `- Post-auth state: token_present=${postAuthState.token_present === true ? "true" : "false"}, auth_cookie_present=${
          postAuthState.auth_cookie_present === true ? "true" : "false"
        }, need_profile=${
          typeof postAuthState.need_profile === "boolean" ? String(postAuthState.need_profile) : "unknown"
        }, serialized_step=${postAuthState.serialized_step || "n/a"}`
      );
    }
    if (Array.isArray(diagnosticDetails.asset_fingerprints) && diagnosticDetails.asset_fingerprints.length) {
      lines.push(
        `- Asset fingerprints: ${diagnosticDetails.asset_fingerprints
          .slice(0, 8)
          .map((asset) => asset.file || asset.url || asset.hash)
          .filter(Boolean)
          .join(", ")}`
      );
    }
    if (Number.isFinite(Number(diagnosticDetails.page_error_count)) || Number.isFinite(Number(diagnosticDetails.failed_request_count))) {
      lines.push(
        `- Runtime errors: page_errors=${Math.max(0, Math.round(Number(diagnosticDetails.page_error_count) || 0))}, console_errors=${Math.max(
          0,
          Math.round(Number(diagnosticDetails.console_error_count) || 0)
        )}, failed_requests=${Math.max(0, Math.round(Number(diagnosticDetails.failed_request_count) || 0))}`
      );
    }
    lines.push(
