...
function fallbackVideoUrls(context) {
  ...
  return uniqueStringList(urls.filter(Boolean), 6, 4096);
}
...
function normalizeEvidence(value, context) {
  ...
  const normalizedScreenshots = screenshots.length ? screenshots : fallbackScreenshots;
  const normalizedVideos = videos.length ? videos : fallbackVideos;
  const proofState = screenshots.length ? "verified" : normalizedScreenshots.length ? "fallback" : "missing";
  const proofSource = screenshots.length ? "explicit_evidence" : normalizedScreenshots.length ? "run_fallback" : "none";
    ...
    return [
      {
        ...
        name: "Could not finish login",
        status: "blocked",
        summary: "The tester reached the login or sign-up step, but the submit button did not work, so the test could not continue.",
        steps: [
          "Open the first page.",
          "Start login or sign-up.",
          "Stop when the submit button does not work."
        ],
        pages: uniqueStringList([runRequest.target_url], 5),
        evidence: {
          screenshots: primaryScreenshots.length ? primaryScreenshots : fallbackScreenshotUrls(context),
...
function buildCoverage(actions = {}, findings = [], runRequest, context = {}) {
  ...
  if (!earlyNavigationFailure && !runRequest?.credentials) {
...
function buildMarkdownReport(report, runRequest, extras = {}) {
  ...
  for (const finding of findings) {
    ...
    if ((evidence.console_logs || []).length) {
      lines.push(`  - Console logs: ${evidence.console_logs.join(", ")}`);
    }
    if ((evidence.network_logs || []).length) {
      lines.push(`  - Network logs: ${evidence.network_logs.join(", ")}`);
    }
