...
function buildSystemPrompt(runRequest) {
  ...
  return [
    ...
    ...(browserMode === "advanced_browser"
      ? [
          "This run is using the advanced browser runtime. Prefer the remote browser session, stronger stealth behavior, and supported captcha-solving paths when the site resists normal automation."
        ]
      : []),
...
function buildTaskPrompt(runRequest) {
  ...
  const browserModeText =
    browserMode === "advanced_browser"
      ? "Browser runtime: Advanced browser (Browserbase with stronger stealth and supported captcha handling)"
