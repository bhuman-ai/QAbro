...
function normalizeImapInbox(raw) {
  const source = isPlainObject(raw)
    ? raw
    : isPlainObject(raw?.inbox)
      ? raw.inbox
  ...
  const email = sanitizeString(source.email || source.address, 320).toLowerCase();
  const username = sanitizeString(source.username || source.user, 320).toLowerCase() || email;
  const host = sanitizeString(source.host || source.imapHost || source.imap_host, 320).toLowerCase();
  const password = sanitizeString(source.password, 1024);
  const accessToken = sanitizeString(source.accessToken || source.access_token, 4096);
  ...
  return {
    ...
    username,
    host,
    port: Number.isFinite(Number(source.port || source.imapPort || source.imap_port))
      ? Math.max(1, Math.min(65535, Math.floor(Number(source.port || source.imapPort || source.imap_port))))
      : 993,
    secure:
      source.secure === undefined
        ? true
        : Boolean(source.secure),
    mailbox: sanitizeString(source.mailbox || source.folder, 120) || "INBOX",
    ...(password ? { password } : {}),
    ...(accessToken ? { accessToken } : {})
  };
}

function buildImapMessageDetail(message = {}) {
  const envelope = isPlainObject(message.envelope) ? message.envelope : {};
  const fromEntry = Array.isArray(envelope.from) && envelope.from[0] ? envelope.from[0] : {};
  ...
  return {
    id: sanitizeString(String(message.uid || message.seq || ""), 128),
    from: sanitizeString(fromEntry.address || "", 320),
    to: toEntries.map((entry) => sanitizeString(entry?.address, 320)).filter(Boolean),
    subject: sanitizeString(envelope.subject || "", 1000),
    intro: sanitizeString(sourceBuffer, 4000),
    text: sanitizeString(sourceBuffer, 100000),
    html: sanitizeString(sourceBuffer, 100000),
    seen: Array.isArray(message.flags) ? message.flags.includes("\\Seen") : false,
    createdAt: toIso(message.internalDate || message.date)
  };
}

function createImapProvider() {
  async function createInbox(identity = {}) {
    const existing = normalizeImapInbox(identity);
    if (!existing) {
      throw new Error("IMAP OTP provider requires email, host, username, and password or access token");
    }
    return existing;
  }

  async function waitForOtpCode(inbox, waitOptions = {}) {
    const normalized = normalizeImapInbox(inbox);
    if (!normalized) {
      throw new Error("IMAP inbox identity is incomplete");
    }
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      try {
        ...
        lock = await client.getMailboxLock(normalized.mailbox || "INBOX");
        const mailboxInfo = await client.mailboxOpen(normalized.mailbox || "INBOX");
        const exists = Number(mailboxInfo?.exists || 0);
        if (exists > 0) {
          const start = Math.max(1, exists - 20 + 1);
          ...
          const selected = selectMostRecentMessage(
            messages.map(buildImapMessageDetail),
            {
              subjectPattern: waitOptions.subjectPattern,
              senderPattern: waitOptions.senderPattern,
              since: waitOptions.since
            }
          );
          if (selected) {
            lastMessage = selected;
            const otpCode = extractBestOtp(selected, {
              minDigits: waitOptions.minDigits,
              maxDigits: waitOptions.maxDigits
            });
            const urls = extractUrls([selected.subject, selected.text, selected.html].join("\n"));
            const matchedLink = waitOptions.linkPattern
              ? urls.find((url) => new RegExp(String(waitOptions.linkPattern), "i").test(url)) || null
              : urls[0] || null;
...
function resolveProvider(providerName, options = {}) {
  ...
  if (["imap", "imapflow"].includes(normalized)) {
    return createImapProvider(options);
  }
