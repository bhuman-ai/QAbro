...
function readEvidenceList(report, kind, payload = null) {
  ...
  const evidenceMedia =
    safePayload.evidence_media && typeof safePayload.evidence_media === "object"
      ...
      : {};
  const mediaEntries = Array.isArray(evidenceMedia[field]) ? evidenceMedia[field] : [];
  for (const entry of mediaEntries) {
    const source = sanitizeString(entry?.source || entry?.path || entry?.raw, 4096);
    if (source) {
      values.push(source);
    }
  }
...
function buildQaEvidenceManifest(report, payload, options = {}) {
    ...
    readEvidenceList(report, kind, payload)
      .map((source, index) => {
        ...
        return {
          ...
          ...(Number.isInteger(entry?.segment_index)
            ? {
  ...
  const firstRecordingSegment = videos
    .filter((item) => Number(item.segment_count) > 1 && Number.isInteger(item.segment_index))
    .sort((left, right) => left.segment_index - right.segment_index)[0];
  const recording =
    firstRecordingSegment ||
