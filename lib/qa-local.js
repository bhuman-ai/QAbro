  ...
  const browserChannel =
    sanitizeOptionalString(
      options.browserChannel ||
        metadata.browser_channel ||
        metadata.local_browser_channel ||
        metadata.qa_local_browser_channel ||
        process.env.QA_LOCAL_BROWSER_CHANNEL ||
        process.env.QA_PLAYWRIGHT_BROWSER_CHANNEL,
      64
    ) || null;
...
async function executeLocalQaRun(runRequest, options = {}) {
  ...
  const report = normalizeReport({
    ...
    reportUrl: publication.reportUrl || null,
