...
function buildManualQaWorkPackets(session = {}, options = {}) {
  ...
  for (const item of items) {
    ...
    const transcriptCandidates = signals.transcriptEvents
      .filter((entry) => entry.text)
      .slice(-4);
    if (packets.length < maxPackets && (item.note || transcriptCandidates.length || videoEvents.length || signals.media.length)) {
...
function buildManualQaAgentFeedbackMarkdown(session, options = {}) {
  ...
  if (agentActionMode === "fix_and_retest") {
    ...
    lines.push("- Do not claim the work is done until the user has approved the preview and the verified fixed version is ready, unless you clearly state the blocker.");
  } else {
