...
function normalizeManualQaSessionRow(row) {
  ...
  if (
    ...
    !hasManualQaRecordingAnalysisConsent(session.qualification_trial || {})
  ) {
    const recordingCount = collectManualQaRecordingMedia(session).length;
    findingsAnalysis = {
      ...findingsAnalysis,
      status: "failed",
      media_count: recordingCount,
      processed_media_count: 0,
      transcript_event_count: 0,
      completed_at: null,
      lease_id: null,
...
async function updateManualQaQualificationTrial(sessionId, trialValue = {}, options = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const now = new Date().toISOString();
    const existingSession = normalizeManualQaSessionRow(loaded.row);
    const grantingRecordingAnalysisConsent = Boolean(
      !hasManualQaRecordingAnalysisConsent(existingTrial) &&
        hasManualQaRecordingAnalysisConsent(nextTrial)
    );
    const resetAnalysis = normalizeManualQaFindingsAnalysis(null, {
      mediaCount: collectManualQaRecordingMedia(existingSession).length
    });
    const nextManual = {
      ...existingManual,
      updated_at: now,
      ...
      ...(grantingRecordingAnalysisConsent
        ? {
            findings_analysis: resetAnalysis,
            findingsAnalysis: null,
            checklist: existingSession.checklist,
            work_packets: [],
            workPackets: null
          }
        : {})
...
async function queueManualQaFindingsAnalysis(sessionId, options = {}) {
  ...
  const hasQualificationTrial = Boolean(session.qualification_trial);
  ...
  if (hasQualificationTrial && !hasManualQaRecordingAnalysisConsent(trial)) {
    const consentRequired = await updateManualQaFindingsAnalysis(
      sessionId,
      {
        ...current,
        analysis_id: current.analysis_id || `recording_${crypto.randomBytes(12).toString("hex")}`,
    ...
    return consentRequired.ok ? { ...consentRequired, queued: false, recordings } : consentRequired;
  }
...
function buildSafeExportSession(session, options = {}) {
  ...
  let currentAnalysis = resolveCurrentManualQaFindingsAnalysis(
    session,
    session.findings_analysis || session.findingsAnalysis
  );
  if (
    qualificationRecordingIsLocked(session) &&
    !hasManualQaRecordingAnalysisConsent(session.qualification_trial || session.qualificationTrial || {})
  ) {
    const recordingCount = collectManualQaRecordingMedia(session).length;
    currentAnalysis = {
      ...currentAnalysis,
...
function buildManualQaCustomerReportMarkdown(session) {
  ...
  if (analysis.status !== "complete") {
    ...
    if (analysis.status === "processing") {
      ...
      lines.push(
        "The tester must approve video-and-transcript analysis from their existing private trial link. No findings have been published from the tester note.",
        ""
      );
    } else if (analysis.status === "failed") {
      lines.push("The recording could not be analyzed. No findings have been published from the tester note.", "");
    } else {
      lines.push("This recording has not been analyzed yet.", "");
    }
