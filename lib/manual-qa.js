...
function normalizeAgentActionMode(input = {}, fallback = "report_only") {
  const explicit = sanitizeString(input.agent_action_mode || input.agentActionMode, 80).toLowerCase();
...
function buildManualQaAgentFeedbackMarkdown(session, options = {}) {
  ...
  const agentActionMode = normalizeAgentActionMode(safe.context || {}, "fix_and_retest");
  const lines = [
  ...
  if (agentActionMode === "fix_and_retest") {
    lines.push("- Mode: report and start work.");
