...
function pushManualQaItemFeedbackLines(lines, item, index) {
  ...
  if (Array.isArray(item.evidence_media) && item.evidence_media.length) {
    lines.push(`- Captured media: ${item.evidence_media.length} file${item.evidence_media.length === 1 ? "" : "s"}`);
  }
  if (item.widget_context?.page_url) {
  ...
  const networkFindings = networkEvents
    .filter((entry) => Number(entry.status) >= 400 || entry.message)
    .slice(-8);
...
function buildManualQaAgentFeedbackMarkdown(session, options = {}) {
  ...
  const lines = [
    ...
    "",
    `- Scope: ${itemId ? "single checklist item" : "all checklist items"}`,
    `- Session: ${safe.session_id || "n/a"}`,
    `- Title: ${safe.title || "Manual QA session"}`,
    `- Target: ${safe.target_url || "n/a"}`,
    `- Overall status: ${safe.status || "unknown"}`,
    `- Counts: ${counts.pass || 0} pass, ${counts.fail || 0} fail, ${counts.confusing || 0} confusing, ${counts.blocked || 0} blocked, ${counts.pending || 0} pending`,
