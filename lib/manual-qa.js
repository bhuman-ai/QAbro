...
const RAW_FINDINGS_ANALYSIS_CATEGORIES = new Set(["bug", "frustration", "frustration_point", "aha_moment", "observation"]);
...
const QUALIFICATION_REPORT_DELIVERY_STATUSES = new Set([
  "disabled",
  "pending",
  "sending",
  "accepted",
...
function normalizeQualificationReportDelivery(value) {
  const delivery = isPlainObject(value) ? value : {};
  const rawStatus = sanitizeString(delivery.status, 40).toLowerCase();
  const status = QUALIFICATION_REPORT_DELIVERY_STATUSES.has(rawStatus)
    ? rawStatus
    : "disabled";
  return {
    status,
    enabled_at: sanitizeOptionalString(delivery.enabled_at || delivery.enabledAt, 128) || null,
    analysis_id: sanitizeOptionalString(delivery.analysis_id || delivery.analysisId, 160) || null,
    recording_fingerprint:
      sanitizeOptionalString(
        delivery.recording_fingerprint || delivery.recordingFingerprint,
        160
      ) || null,
    delivery_key: sanitizeOptionalString(delivery.delivery_key || delivery.deliveryKey, 160) || null,
    recipient: sanitizeOptionalString(delivery.recipient, 320)?.toLowerCase() || null,
    attempt_count: Math.max(
      0,
      Math.min(20, Math.round(Number(delivery.attempt_count || delivery.attemptCount || 0) || 0))
    ),
    lease_id: sanitizeOptionalString(delivery.lease_id || delivery.leaseId, 160) || null,
    lease_expires_at:
      sanitizeOptionalString(delivery.lease_expires_at || delivery.leaseExpiresAt, 128) || null,
    last_attempt_at:
      sanitizeOptionalString(delivery.last_attempt_at || delivery.lastAttemptAt, 128) || null,
    accepted_at: sanitizeOptionalString(delivery.accepted_at || delivery.acceptedAt, 128) || null,
    failed_at: sanitizeOptionalString(delivery.failed_at || delivery.failedAt, 128) || null,
    next_retry_at: sanitizeOptionalString(delivery.next_retry_at || delivery.nextRetryAt, 128) || null,
    app_message_id:
      sanitizeOptionalString(delivery.app_message_id || delivery.appMessageId, 512) || null,
    provider_message_id:
      sanitizeOptionalString(delivery.provider_message_id || delivery.providerMessageId, 512) || null,
    provider_response:
      sanitizeOptionalString(delivery.provider_response || delivery.providerResponse, 1200) || null,
    error_code: sanitizeOptionalString(delivery.error_code || delivery.errorCode, 160) || null,
    retryable:
...
function normalizeQualificationTrial(value, options = {}) {
  ...
  const leadRating = isPlainObject(value.lead_rating || value.leadRating)
    ...
    : {};
  const reportDelivery = normalizeQualificationReportDelivery(
    value.report_delivery || value.reportDelivery
  );
  ...
  const normalized = {
    ...
    assignment: {
      type: assignmentType,
      tester_reward_type:
        sanitizeString(assignment.tester_reward_type || assignment.testerRewardType, 40).toLowerCase() === "qa_credit"
          ? "qa_credit"
          : "cash",
      tester_pay_cents:
        assignmentType === "paid"
    ...
    tester: {
      ...
      public_name:
        sanitizeOptionalString(tester.public_name || tester.publicName, 80) || null,
  ...
  if (includeAccess) {
    ...
    normalized.access = {
      ...
      lead_report_token_hashes: normalizeStringList(
        access.lead_report_token_hashes || access.leadReportTokenHashes,
        5,
        256
      ),
...
function redactSensitiveText(value, maxLength = 1000) {
  ...
  return redactSensitiveUrl(raw)
    .replace(/\b(bud_(?:widget|trial)_|mcp_)[A-Za-z0-9._-]+/gi, "$1[redacted]")
...
function normalizeFindingsAnalysisFinding(value = {}, index = 0) {
  ...
  const title =
    redactSensitiveText(source.title || source.name, 180) ||
  ...
  return {
    ...
    category,
    title,
    summary: redactSensitiveText(source.summary || source.description, 1600) || null,
    suggested_fix:
      category === "bug" || category === "frustration_point"
        ? redactSensitiveText(
            source.suggested_fix || source.suggestedFix || source.suggestion ||
              source.recommendation || source.fix_hint || source.fixHint,
            1200
          ) || null
        : null,
    ...
    support_verified: source.support_verified === true ? true : null
  };
}
...
function normalizeManualQaAiUsage(value) {
  if (!isPlainObject(value) || value.tracking_available !== true) return null;
  const count = (entry) => {
    const number = Number(entry);
    return Number.isFinite(number) && number >= 0 ? Math.round(number) : 0;
  };
  const rawCost = value.total_cost_usd ?? value.totalCostUsd ?? value.cost_usd ?? value.cost;
  const numericCost = Number(rawCost);
  const totalCostUsd = rawCost !== null && rawCost !== undefined && rawCost !== "" && Number.isFinite(numericCost) && numericCost >= 0
    ? numericCost
    : null;
  const pricedRequestCount = count(value.priced_request_count ?? value.pricedRequestCount);
  const unpricedResponseCount = count(value.unpriced_response_count ?? value.unpricedResponseCount);
  const uncertainRequestCount = count(value.uncertain_request_count ?? value.uncertainRequestCount);
  const requestCount = Math.max(
    count(value.request_count ?? value.requestCount),
    pricedRequestCount + unpricedResponseCount + uncertainRequestCount
  );
  return {
    provider: sanitizeOptionalString(value.provider, 80) || "openrouter",
    currency: "USD",
    ...
    prompt_tokens: count(value.prompt_tokens ?? value.promptTokens),
    completion_tokens: count(value.completion_tokens ?? value.completionTokens),
    total_tokens: count(value.total_tokens ?? value.totalTokens)
  };
}
...
function normalizeManualQaFindingsAnalysis(value = {}, options = {}) {
  ...
  return {
    ...
    aggregation_model:
      sanitizeOptionalString(source.aggregation_model || source.aggregationModel, 160) || null,
    verification_model:
      sanitizeOptionalString(source.verification_model || source.verificationModel, 160) || null,
    semantic_verification_version:
      Number.isInteger(Number(source.semantic_verification_version ?? source.semanticVerificationVersion)) &&
      Number(source.semantic_verification_version ?? source.semanticVerificationVersion) > 0
        ? Number(source.semantic_verification_version ?? source.semanticVerificationVersion)
        : null,
    ai_usage: normalizeManualQaAiUsage(source.ai_usage || source.aiUsage),
...
function validateCurrentCompleteFindingsAnalysis(analysis = {}, recordings = []) {
  ...
  for (const finding of analysis.findings) {
    if (analysis.semantic_verification_version && finding.support_verified !== true) {
      return { ok: false, error_code: "finding_semantic_unverified" };
    }
...
function normalizeManualWorkPacket(value = {}, index = 0, session = {}) {
  ...
  return {
    ...
    suggested_fix:
      redactSensitiveText(
        source.suggested_fix || source.suggestedFix || source.suggestion || source.recommendation,
        1200
      ) || null,
    item_id: itemId,
...
function resolveManualQaTopicSegmenterConfig(options = {}) {
  ...
  const configuredModel = sanitizeString(
    options.topicSegmenterModel ||
      ...
      (openRouterDefault ? "openai/gpt-4.1-nano" : "gpt-4.1-nano"),
    256
  );
  const model = ["gpt-4.1-mini", "openai/gpt-4.1-mini"].includes(configuredModel.toLowerCase())
    ? (openRouterDefault ? "openai/gpt-4.1-nano" : "gpt-4.1-nano")
    : configuredModel;
  return {
...
async function updateManualQaQualificationTrial(sessionId, trialValue = {}, options = {}) {
  ...
  const leadRatingOnlyMutation = options.leadRatingOnlyMutation === true;
  const testerPublicNameOnlyMutation = options.testerPublicNameOnlyMutation === true;
  if (leadRatingOnlyMutation && testerPublicNameOnlyMutation) {
    return {
      ok: false,
      status: 400,
      error: "Only one protected qualification-trial field can be updated at a time"
    };
  }
  if (leadRatingOnlyMutation) {
    const hasFence = [
      "expectedLeadRatingScore",
      "expectedLeadRatingNote",
      "expectedLeadRatingRatedAt"
    ].every((key) => Object.prototype.hasOwnProperty.call(options, key));
    if (options.allowLeadRatingMutation !== true || !hasFence) {
      return { ok: false, status: 403, error: "Customer-rating mutation access and fence are required" };
    }
    for (const key of Object.keys(intentPatch)) {
      if (key !== "lead_rating") delete intentPatch[key];
    }
    intentPatch.lead_rating = requestedTrial.lead_rating;
  } else if (testerPublicNameOnlyMutation) {
    if (
      options.allowTesterPublicNameUpdate !== true ||
      !Object.prototype.hasOwnProperty.call(options, "expectedTesterPublicName")
    ) {
      return { ok: false, status: 403, error: "Tester public-name mutation access and fence are required" };
    }
    for (const key of Object.keys(intentPatch)) {
      if (key !== "tester") delete intentPatch[key];
    }
    intentPatch.tester = { public_name: requestedTrial.tester.public_name };
  } else {
    delete intentPatch.lead_rating;
    if (isPlainObject(intentPatch.tester)) {
      delete intentPatch.tester.public_name;
      if (!Object.keys(intentPatch.tester).length) delete intentPatch.tester;
    }
  }
  const hasReportDeliveryFence = [
    "expectedReportDeliveryStatus",
    "expectedReportDeliveryLeaseId",
    "expectedReportDeliveryAnalysisId"
  ].every((key) => Object.prototype.hasOwnProperty.call(options, key));
  const allowReportDeliveryMutation =
    options.allowReportDeliveryMutation === true && hasReportDeliveryFence;
  const allowLeadReportTokenHashesMutation =
    options.allowLeadReportTokenHashesMutation === true && allowReportDeliveryMutation;
  const reportDeliveryOnlyMutation =
    options.reportDeliveryOnlyMutation === true && allowReportDeliveryMutation;
  if (reportDeliveryOnlyMutation) {
    for (const key of Object.keys(intentPatch)) {
      if (key !== "report_delivery" && key !== "access") delete intentPatch[key];
    }
    if (isPlainObject(intentPatch.access)) {
      for (const key of Object.keys(intentPatch.access)) {
        if (key !== "lead_report_token_hashes") delete intentPatch.access[key];
      }
      if (!Object.keys(intentPatch.access).length) delete intentPatch.access;
    }
  }
  if (!allowReportDeliveryMutation) {
    delete intentPatch.report_delivery;
  }
  if (!allowLeadReportTokenHashesMutation && isPlainObject(intentPatch.access)) {
    delete intentPatch.access.lead_report_token_hashes;
    if (!Object.keys(intentPatch.access).length) delete intentPatch.access;
  }
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (leadRatingOnlyMutation) {
      const existingRating = isPlainObject(existingTrial.lead_rating) ? existingTrial.lead_rating : {};
      const expectedScore =
        options.expectedLeadRatingScore === null ||
        options.expectedLeadRatingScore === undefined ||
        options.expectedLeadRatingScore === ""
          ? null
          : Math.max(1, Math.min(5, Math.round(Number(options.expectedLeadRatingScore))));
      const expectedNote = sanitizeOptionalString(options.expectedLeadRatingNote, 2400) || null;
      const expectedRatedAt = sanitizeOptionalString(options.expectedLeadRatingRatedAt, 128) || null;
      if (
        existingRating.score !== expectedScore ||
        (existingRating.note || null) !== expectedNote ||
        (existingRating.rated_at || null) !== expectedRatedAt
      ) {
        return {
          ok: false,
          status: 409,
          error: "Customer rating changed before it could be updated",
          trial: normalizeQualificationTrial(existingTrial)
        };
      }
    }
    if (testerPublicNameOnlyMutation) {
      const expectedPublicName = sanitizeOptionalString(options.expectedTesterPublicName, 80) || null;
      if ((existingTrial.tester?.public_name || null) !== expectedPublicName) {
        return {
          ok: false,
          status: 409,
          error: "Tester public name changed before it could be updated",
          trial: normalizeQualificationTrial(existingTrial)
        };
      }
    }
    const existingDelivery = normalizeQualificationReportDelivery(existingTrial.report_delivery);
    const expectedDeliveryChecks = [
      ["expectedReportDeliveryStatus", existingDelivery.status, 40],
      ["expectedReportDeliveryLeaseId", existingDelivery.lease_id, 160],
      ["expectedReportDeliveryAnalysisId", existingDelivery.analysis_id, 160]
    ];
    const deliveryFenceChanged = expectedDeliveryChecks.some(([key, currentValue, maxLength]) => {
      if (!Object.prototype.hasOwnProperty.call(options, key)) return false;
      return sanitizeString(options[key], maxLength) !== sanitizeString(currentValue, maxLength);
    });
    if (deliveryFenceChanged) {
      return {
        ok: false,
        status: 409,
        error: "Report delivery state changed before it could be updated",
        trial: normalizeQualificationTrial(existingTrial)
      };
    }
    const nextTrial = normalizeQualificationTrial(
    ...
    const currentDeliveredAtMs = Date.parse(loaded.row.delivered_at || "");
    const nextDeliveredAtMs = Number.isFinite(currentDeliveredAtMs)
      ? Math.max(Date.now(), currentDeliveredAtMs + 1)
      : Date.now();
    const now = new Date(nextDeliveredAtMs).toISOString();
    const nextManual = {
      ...existingManual,
      updated_at: now,
      qualification_trial: {
...
function validateRawCompleteFindingsAnalysis(value = {}, recordings = []) {
  ...
  for (const finding of findings) {
    ...
    const rawSuggestedFix = finding?.suggested_fix ?? finding?.suggestedFix;
    if (
      ...
      (rawSuggestedFix !== undefined && rawSuggestedFix !== null &&
        (typeof rawSuggestedFix !== "string" || !rawSuggestedFix.trim())) ||
...
async function queueManualQaFindingsAnalysis(sessionId, options = {}) {
  ...
  const sameRecording = Boolean(fingerprint && persistedCurrent.recording_fingerprint === fingerprint);
  ...
  if (!inspectedRecordings.ok) {
    const invalidSet = await updateManualQaFindingsAnalysis(
      sessionId,
      {
        ...
        attempt_count: 0,
        recording_fingerprint: fingerprint,
        model: sameRecording ? current.model : null,
        aggregation_model: sameRecording ? current.aggregation_model : null,
        verification_model: sameRecording ? current.verification_model : null,
        semantic_verification_version: null,
        ai_usage: sameRecording ? current.ai_usage : null,
        clip_results: [],
        findings: []
      },
      {
  ...
  if (!recordings.length) {
    const missing = await updateManualQaFindingsAnalysis(
      ...
      {
        ...current,
        analysis_id: `recording_${crypto.randomBytes(12).toString("hex")}`,
        status: "failed",
        media_count: 0,
        processed_media_count: 0,
        transcript_event_count: 0,
...
function buildCustomerRecordingWorkPacket(packet = {}) {
  ...
  return {
    ...
    suggested_fix: redactSensitiveText(normalized.suggested_fix, 1200) || null,
...
function stripManualQaAnalysisInternals(value) {
  if (!isPlainObject(value)) return value || null;
  const {
    analysis_id: _analysisId,
    analysisId: _legacyAnalysisId,
    lease_id: _leaseId,
    leaseId: _legacyLeaseId,
    lease_expires_at: _leaseExpiresAt,
    leaseExpiresAt: _legacyLeaseExpiresAt,
    attempt_count: _attemptCount,
    attemptCount: _legacyAttemptCount,
    recording_fingerprint: _recordingFingerprint,
    recordingFingerprint: _legacyRecordingFingerprint,
    model: _model,
    aggregation_model: _aggregationModel,
    aggregationModel: _legacyAggregationModel,
    verification_model: _verificationModel,
    verificationModel: _legacyVerificationModel,
    semantic_verification_version: _semanticVerificationVersion,
    semanticVerificationVersion: _legacySemanticVerificationVersion,
    ai_usage: _aiUsage,
    aiUsage: _legacyAiUsage,
    ...presentationAnalysis
  } = value;
  return presentationAnalysis;
}

function buildManualQaReportSessionView(session) {
  if (!isPlainObject(session)) return session || null;
  const {
  ...
  return {
    ...presentationSession,
    findings_analysis: stripManualQaAnalysisInternals(
      session.findings_analysis || session.findingsAnalysis
    )
  };
}
    ...
    const customerTrial = trial
      ? {
          ...
          assignment: {
            ...
            tester_reward_type: trial.assignment?.tester_reward_type,
...
function buildManualQaCustomerReportMarkdown(session) {
  ...
  const problems = packets.filter((packet) => packet.category === "bug" || packet.category === "frustration_point");
  const wins = packets.filter((packet) => packet.category === "aha_moment");
  const observations = packets.filter((packet) => packet.category === "observation");
  const suggestedFixes = problems.filter((packet) => sanitizeString(packet.suggested_fix, 1200));
  const lines = [
  ...
  const pushFinding = (packet, typeLabel = "") => {
      const summary = customerFindingSummary(packet);
      lines.push(`- **${typeLabel ? `${typeLabel}: ` : ""}${packet.title || "Captured point"}**`);
  ...
  if (!packets.length) {
    lines.push("No clear findings were identified in the recording.", "");
  }

  if (problems.length) {
    lines.push("### Problems", "");
    for (const packet of problems) {
      pushFinding(packet, packet.category === "bug" ? "Bug" : "Friction");
    }
    lines.push("");
  }
  ...
  if (wins.length) {
    ...
    for (const packet of wins) pushFinding(packet);
    lines.push("");
  }

  if (suggestedFixes.length) {
    lines.push("### Suggested fixes", "");
    lines.push("AI recommendations based on the recording-backed problems above.", "");
    for (const packet of suggestedFixes) {
      lines.push(`- ${packet.suggested_fix}`);
      lines.push(`  For: ${packet.title || "Recorded problem"}`);
    }
    lines.push("");
  }

  if (observations.length) {
    lines.push("### More observations", "");
...
module.exports = {
  ...
  __private: {
    ...
    normalizeQualificationReportDelivery,
