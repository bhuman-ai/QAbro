...
async function recordManualQaAgentFeedback(verified, feedback = {}, options = {}) {
  ...
  const nextPackage = normalizeManualAgentFeedbackPackage({
    ...feedback,
...
function buildManualQaReportMarkdown(session) {
  ...
  const lines = [
    ...
    `- ${safe.review_mode === "freestyle" ? "Freestyle capture" : "Checklist"}: ${counts.pass || 0} pass, ${counts.fail || 0} fail, ${counts.confusing || 0} confusing, ${counts.blocked || 0} blocked, ${counts.pending || 0} pending`,
