...
function buildBrowserState(targetUrl, existingArtifacts = {}) {
  ...
  return {
    mode: "hosted_popout_chromium",
    status: viewerUrl || embedUrl ? "viewer_ready" : "not_configured",
    ...
    note:
      viewerUrl || embedUrl
        ? "Open the hosted Chromium viewer, load the target URL there, and use the checklist beside it."
        : "Hosted Chromium viewer is not configured for this environment."
  };
}
...
function buildManualQaReportMarkdown(session) {
  ...
  lines.push(`- Mode: ${safe.browser?.mode || "hosted_popout_chromium"}`);
  lines.push(`- Viewer status: ${safe.browser?.status || "unknown"}`);
  lines.push("- Viewer credentials and sensitive URL parameters are redacted from this export.");
  lines.push("");
