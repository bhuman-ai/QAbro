...
function buildManualWidgetInstall(session, token, publicBaseUrl) {
  ...
  return {
    ...
    install_note: "Inject this script into the preview/dev build so BeforeUsersDo appears directly on the tested page."
  };
}
...
function buildBrowserState(targetUrl, existingArtifacts = {}) {
  ...
  return {
    ...
    note:
      hasRemoteFallback
        ? "Primary review uses the agent-injected page widget on the tester's own browser. A sidecar recorder and remote browser fallback remain available under Advanced."
        : "Primary review uses the agent-injected page widget on the tester's own browser, with the sidecar recorder as fallback."
  };
}
...
function normalizeManualQaSessionRow(row) {
  ...
  return {
    ...
    widget: {
      ...
      status: rawWidget.token_hash ? "ready" : "not_configured",
      installed: rawWidget.installed === true,
      note:
