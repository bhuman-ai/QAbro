...
function mergeEvidenceMediaLists(existing, incoming, maxItems = MAX_WIDGET_MEDIA_ITEMS) {
  const merged = new Map();
  for (const entry of normalizeEvidenceMediaList([...(existing || []), ...(incoming || [])], maxItems * 2)) {
    const key = evidenceMediaKey(entry) || JSON.stringify(entry);
    merged.set(key, entry);
  }
  return Array.from(merged.values()).slice(-maxItems);
}
...
function normalizeWorkPacketCategory(value) {
  ...
  if (category === "aha" || category === "positive" || category === "delight") {
    return "aha_moment";
  }
  ...
  for (const event of orderedEvents) {
      ...
      session.checklist = session.checklist.map((item) => {
        ...
        if (Object.prototype.hasOwnProperty.call(patch, "widget_context") || Object.prototype.hasOwnProperty.call(patch, "widgetContext")) {
          next.widget_context = mergeManualWidgetContexts(
            next.widget_context,
            patch.widget_context || patch.widgetContext
          );
        }
        return next;
      });
    ...
    if (eventType === "evidence_added" && itemId) {
      ...
      session.checklist = session.checklist.map((item) => {
        ...
        return {
          ...item,
          evidence_media: mergeEvidenceMediaLists(item.evidence_media, [evidence], MAX_WIDGET_MEDIA_ITEMS),
...
async function verifyManualQaWidgetToken(sessionId, token, options = {}) {
  ...
  const testerTrialTokenMatches =
    sanitizeString(qualificationTrial.kind, 80) === "tester_qualification" &&
...
async function updateManualQaQualificationTrial(sessionId, trialValue = {}, options = {}) {
  ...
  const existingPayload = isPlainObject(loaded.row.payload) ? loaded.row.payload : {};
  const existingManual = isPlainObject(existingPayload.manual_qa) ? existingPayload.manual_qa : {};
  const nextTrial = normalizeQualificationTrial(trialValue, {
    includeAccess: true,
...
async function appendManualQaItemEvidence(sessionId, itemId, evidence = {}, options = {}) {
  ...
  const normalizedMedia = mergeEvidenceMediaLists(currentMedia, [normalizedEvidence], MAX_WIDGET_MEDIA_ITEMS);
...
function buildManualQaCustomerReportMarkdown(session) {
  ...
  const packets = (Array.isArray(safe.work_packets) ? safe.work_packets : [])
    .filter((packet) => packet.status !== "dismissed" && sanitizeString(packet.title || packet.summary, 2000));
  ...
  const qualification = isPlainObject(safe.qualification_trial?.qualification)
    ? safe.qualification_trial.qualification
    : {};
  const hasScore = qualification.score !== null && qualification.score !== undefined && qualification.score !== "" && Number.isFinite(Number(qualification.score));
  const reviewed = qualification.status === "verified" || hasScore;
  const reportStatus = recordings.length ? (reviewed ? "Reviewed" : "Needs review") : "Recording missing";
  const submittedAt = safe.qualification_trial?.submitted_at || safe.completed_at || safe.updated_at || null;
  const findingGroups = [
    { category: "bug", label: "Bugs", empty: reviewed ? "No product bugs were captured." : "No product bugs captured yet." },
    { category: "frustration_point", label: "Frustrations", empty: reviewed ? "No frustration points were captured." : "No frustration points captured yet." },
    { category: "aha_moment", label: "Aha moments", empty: reviewed ? "No aha moments were captured." : "No aha moments captured yet." }
  ];
  const lines = [
    ...
    !reviewed ? "- Findings: Draft — review before sharing." : "",
    "",
...
async function exportManualQaSession(sessionId, options = {}) {
  ...
  return {
    ...
    session: safeSession,
    markdown:
      options.view === "customer" || options.view === "human_report"
