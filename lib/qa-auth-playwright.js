...
async function isLocatorActionable(locator, timeout = 1200) {
...
async function firstVisibleLocator(locators, timeout = 1200) {
  ...
  for (const locator of Array.isArray(locators) ? locators : []) {
    ...
    for (const candidate of candidates) {
      ...
      if (await isLocatorActionable(candidate, Math.min(timeout, 600))) {
        return candidate;
      }
    }
  }
...
function buildAuthLocators(page) {
  return {
    authEntry: [
      page.getByRole("button", { name: /sign\s*in|log\s*in|continue/i }),
      page.getByRole("link", { name: /sign\s*in|log\s*in|continue/i }),
      page.locator(
        [
    ...
    signupEntry: [
      page.getByRole("button", { name: /sign\s*up|create account|register|start free|try free/i }),
      page.getByRole("link", { name: /sign\s*up|create account|register|start free|try free/i }),
      page.locator(
        [
    ...
    username: [
      ...
      page.locator(
        [
          ...
          'input[type="text"]:not([placeholder*="search" i]):not([name*="search" i]):not([id*="search" i])',
          'input:not([type]):not([placeholder*="search" i]):not([name*="search" i]):not([id*="search" i])'
        ].join(",")
      )
    ],
    password: [
...
async function performCredentialedLogin(page, runRequest, options = {}) {
  ...
  if (preferGoogle) {
    ...
    const usernameInput = emailInput || (await firstVisibleLocator(locators.username, 1200));
