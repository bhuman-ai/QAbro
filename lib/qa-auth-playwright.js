...
async function restoreAuthFormValuesAfterCaptcha(page, locators, values = {}, options = {}) {
  ...
  const fullNameInput = values.autoCreateAccount ? await firstVisibleLocator(locators.fullName, 1200) : null;
  ...
  const usernameFallbackInput =
    !emailInput && !usernameInput && !fullNameInput
  ...
  if (values.autoCreateAccount && fullNameInput) {
...
async function signupModeVisible(page) {
  ...
  if (fullNameInput || confirmPasswordInput || signUpSubmit) {
    return true;
  }
...
async function performCredentialedLogin(page, runRequest, options = {}) {
  ...
  pushRunLog(runLog, "auth_surface_ready", {
    mode: (await signupModeVisible(page)) ? "signup" : "login",
    url: sanitizeString(page.url(), 4096) || null
  });
  ...
  if (preferGoogle) {
    ...
    if (autoCreateAccount && fullNameInput) {
      ...
      await fillBestEffort(fullNameInput, fullNameValue, { timeout: 6000 });
    }
    ...
    const authModeBeforeSubmit = autoCreateAccount && (await signupModeVisible(page)) ? "signup" : "login";
    ...
    const captchaUiVisible = await detectCaptchaRequiredUi(page);
    if (captchaUiVisible && typeof options.resolveCaptcha === "function") {
