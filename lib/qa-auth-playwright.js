...
function buildGeneratedPassword(runRequest, metadata = {}) {
  return (
    ...
    extractPasswordInstruction(runRequest, metadata) ||
    sanitizeOptionalString(process.env.QA_DEFAULT_ACCOUNT_PASSWORD, 256) ||
    `SwarmTest!${String(Date.now()).slice(-6)}${randomToken(4)}A`
  );
}

function cleanExtractedPassword(value) {
  const safe = sanitizeOptionalString(value, 256);
  if (!safe) {
    return null;
  }
  return safe
...
function extractPasswordInstruction(runRequest, metadata = {}) {
  const textParts = [
    metadata.task_to_try,
    metadata.taskToTry,
    metadata.default_auth_password_instruction,
    metadata.defaultAuthPasswordInstruction,
    runRequest?.task_to_try,
    runRequest?.taskToTry,
    ...(Array.isArray(runRequest?.scenario_list) ? runRequest.scenario_list : []),
    ...(Array.isArray(runRequest?.scenarioList) ? runRequest.scenarioList : [])
  ]
    .map((part) => sanitizeOptionalString(part, 1000))
    .filter(Boolean);
  ...
  const patterns = [
    /\buse\s+(?:the\s+)?password\s+["'`]?([^\s"'`,;]+)/i,
    /\buse\s+(?:exactly\s+)?["'`]?([A-Za-z0-9!@#$%^&*()_+\-=[\]{}|\\:<>?/~.]{4,128})["'`]?\s+(?:for|in)\s+(?:both\s+)?(?:the\s+)?(?:password|confirm password)/i,
    /\b(?:password|auth password|default password)\s*[:=]\s*["'`]?([^\s"'`,;]+)/i
  ];

  for (const pattern of patterns) {
    const match = haystack.match(pattern);
    const password = cleanExtractedPassword(match?.[1]);
    if (password && !/^(required|missing|field|input|blank|empty)$/i.test(password)) {
      return password;
    }
  }
  return null;
...
function buildGeneratedCompany(runRequest, metadata = {}) {
  return (
    sanitizeOptionalString(
      metadata.default_auth_company ||
        metadata.defaultAuthCompany ||
        process.env.QA_DEFAULT_ACCOUNT_COMPANY,
      128
    ) ||
    "Before Users Do QA"
  );
...
function buildGeneratedWebsite(runRequest, metadata = {}) {
  return (
    sanitizeOptionalString(
      metadata.default_auth_website ||
        metadata.defaultAuthWebsite ||
        process.env.QA_DEFAULT_ACCOUNT_WEBSITE,
      256
    ) ||
...
function buildGeneratedAuthFieldValues(runRequest, metadata = {}, baseValues = {}) {
  return {
    fullName: baseValues.fullName || buildGeneratedFullName(runRequest, metadata),
    firstName: baseValues.firstName || splitGeneratedName(baseValues.fullName || buildGeneratedFullName(runRequest, metadata)).firstName,
    lastName: baseValues.lastName || splitGeneratedName(baseValues.fullName || buildGeneratedFullName(runRequest, metadata)).lastName,
    phone: baseValues.phone || buildGeneratedPhone(runRequest, metadata),
    username: baseValues.username || "",
    accountHandle: baseValues.accountHandle || "",
    password: baseValues.password || "",
    company: baseValues.company || buildGeneratedCompany(runRequest, metadata),
    website: baseValues.website || buildGeneratedWebsite(runRequest, metadata),
    jobTitle: baseValues.jobTitle || "QA Tester",
    role: baseValues.role || "Founder",
    city: baseValues.city || "San Francisco",
    state: baseValues.state || "CA",
    postalCode: baseValues.postalCode || "94105",
    country: baseValues.country || "United States",
    address: baseValues.address || "123 Test St",
    industry: baseValues.industry || "Software",
    teamSize: baseValues.teamSize || "10",
    genericText: baseValues.genericText || "QA test",
    genericNumber: baseValues.genericNumber || "1"
  };
}
...
function combinedAuthFieldText(field = {}) {
  const safeField = isPlainObject(field) ? field : {};
  return [
    safeField.label,
    safeField.name,
    safeField.id,
    ...
    safeField.title,
    safeField.ariaLabel,
    safeField.ariaLabelledBy,
    safeField.nearbyText,
    safeField.type,
    safeField.inputMode
  ]
...
function authFieldSummary(field = {}, extra = {}) {
  const safeField = isPlainObject(field) ? field : {};
  return {
    label: sanitizeOptionalString(safeField.label, 120) || null,
    name: sanitizeOptionalString(safeField.name, 120) || null,
    id: sanitizeOptionalString(safeField.id, 120) || null,
    type: sanitizeOptionalString(safeField.type, 64) || null,
    tag: sanitizeOptionalString(safeField.tagName, 32) || null,
    required: safeField.required === true,
    ...extra
  };
}

function authFieldHasValue(field = {}) {
  const tagName = sanitizeString(field?.tagName, 32).toLowerCase();
  const type = sanitizeString(field?.type, 64).toLowerCase();
  if (type === "checkbox" || type === "radio") {
    return field?.checked === true;
  }
  const value = sanitizeOptionalString(field?.value, 512) || "";
  if (!value) {
    return false;
  }
  if (tagName === "select") {
    const selected = Array.isArray(field?.options)
      ? field.options.find((option) => option?.selected === true)
      : null;
    const selectedText = normalizeFieldText(selected?.text || value);
    if (!selectedText || /^(select|choose|please select|--|none|n\/a)$/.test(selectedText)) {
      return false;
    }
  }
  return true;
...
function authFieldIsSensitive(field = {}) {
  const combined = combinedAuthFieldText(field);
  if (!combined) {
    return false;
  }
  return /\b(payment|billing|credit card|card number|cardholder|debit|card|cvc|cvv|ccv|expiry|expiration|routing|bank account|iban|swift|ssn|social security|tax id|ein|passport|driver'?s? license|date of birth|birthdate|birthday|security question|security answer|verification code|one time code|one-time code|otp|2fa|mfa|captcha|recaptcha|hcaptcha|promo code|coupon|gift card)\b/.test(combined);
}

function authFieldIsIgnorable(field = {}) {
  const tagName = sanitizeString(field?.tagName, 32).toLowerCase();
  const type = sanitizeString(field?.type, 64).toLowerCase();
  const combined = combinedAuthFieldText(field);
  if (field?.disabled === true || field?.readOnly === true) {
    return true;
  }
  if (["hidden", "button", "submit", "reset", "image", "file", "range", "color"].includes(type)) {
    return true;
  }
  if (tagName === "button") {
    return true;
  }
  if (/\b(search|filter|coupon|promo|referral|invite code)\b/.test(combined)) {
    return true;
  }
  return false;
}

function isTextLikeAuthField(field = {}) {
  const tagName = sanitizeString(field?.tagName, 32).toLowerCase();
  const type = sanitizeString(field?.type || "text", 64).toLowerCase();
  return (
    tagName === "textarea" ||
    tagName === "select" ||
    [
...
function selectOptionForAuthField(field = {}, desiredValue = "") {
  ...
  const candidates = options.filter((option) => {
    ...
    const value = sanitizeOptionalString(option.value, 256) || "";
    const text = sanitizeOptionalString(option.text, 256) || "";
    const normalized = normalizeFieldText(text || value);
    if (!value && !text) {
      return false;
    }
    if (!normalized || /^(select|choose|please select|--|none|n\/a)$/.test(normalized)) {
      return false;
    }
    return true;
  });
  ...
  if (desired) {
    const exact = candidates.find((option) => {
      const value = normalizeFieldText(option.value);
      const text = normalizeFieldText(option.text);
      return value === desired || text === desired;
    });
    if (exact) {
      return exact.value;
    }
    const partial = candidates.find((option) => {
      const value = normalizeFieldText(option.value);
      const text = normalizeFieldText(option.text);
      return value.includes(desired) || desired.includes(value) || text.includes(desired) || desired.includes(text);
    });
    if (partial) {
...
function resolveAuthFieldFill(field = {}, values = {}, options = {}) {
  if (!isPlainObject(field) || !isPlainObject(values)) {
    return null;
  }
  const tagName = sanitizeString(field.tagName, 32).toLowerCase();
  const type = sanitizeString(field.type || (tagName === "select" ? "select" : "text"), 64).toLowerCase();
  const combined = combinedAuthFieldText(field);
  const signupMode = options.signupMode === true;
  const required = field.required === true;
  const fillOptionalSafeFields = options.fillOptionalSafeFields !== false && signupMode;

  if (authFieldIsIgnorable(field)) {
    return null;
  }
  if (authFieldIsSensitive(field)) {
    return {
      key: "sensitive",
      unsafe: true,
      required,
  ...
  if (type === "checkbox") {
    if (
      required &&
      /\b(agree|accept|terms|privacy|policy|consent)\b/.test(combined) &&
      !/\b(newsletter|marketing|updates|subscribe)\b/.test(combined)
    ) {
      return { key: "required_checkbox", mode: "check", value: true };
    }
    return null;
  }
  if (type === "radio") {
  ...
  const shouldFill = required || fillOptionalSafeFields;
  const textLike = isTextLikeAuthField(field);
  const asSelect = tagName === "select";
  const withValue = (key, value) => {
    if (!value && !asSelect) {
      return null;
    }
    return {
      key,
      value,
      mode: asSelect ? "select" : "fill"
    };
  };

  if (type === "email" || /\b(e mail|email|work email|email address)\b/.test(combined)) {
    return withValue("email", values.username);
  }
  if (/\b(confirm|confirmation|repeat|re enter|retype)\b/.test(combined) && /\b(password|passcode)\b/.test(combined)) {
    return withValue("confirm_password", values.password);
  }
  if (type === "password" || /\b(password|passcode)\b/.test(combined)) {
    return withValue("password", values.password);
  }
  if (type === "tel" || /\b(phone|mobile|telephone|tel)\b/.test(combined)) {
    return withValue("phone", values.phone);
  }
  if (/\b(first name|given name|forename)\b/.test(combined)) {
    return withValue("first_name", values.firstName);
  }
  if (/\b(last name|family name|surname)\b/.test(combined)) {
    return withValue("last_name", values.lastName);
  }
  if (type === "url" || /\b(website|web site|site url|company url|domain|homepage|url)\b/.test(combined)) {
    return withValue("website", values.website);
  }
  if (/\b(company|organization|organisation|business|agency|workspace|team name|school)\b/.test(combined)) {
    return withValue("company", values.company);
  }
  if (/\b(job title|title|position)\b/.test(combined)) {
    return withValue("job_title", values.jobTitle);
  }
  if (/\b(role|i am a|i'm a|persona)\b/.test(combined)) {
    return withValue("role", values.role);
  }
  if (/\b(industry|vertical|sector|category)\b/.test(combined)) {
    return withValue("industry", values.industry);
  }
  if (/\b(company size|team size|employees|headcount|number of people|people on your team)\b/.test(combined)) {
    return withValue("team_size", values.teamSize);
  }
  if (/\b(country|nation)\b/.test(combined)) {
    return withValue("country", values.country);
  }
  if (/\b(address|street)\b/.test(combined)) {
    return withValue("address", values.address);
  }
  if (/\b(city|town)\b/.test(combined)) {
    return withValue("city", values.city);
  }
  if (/\b(state|province|region)\b/.test(combined)) {
    return withValue("state", values.state);
  }
  if (/\b(zip|postal|postcode|post code)\b/.test(combined)) {
    return withValue("postal_code", values.postalCode);
  }
  if (/\b(username|user name|handle|login)\b/.test(combined)) {
    return withValue("username", values.accountHandle || values.username);
  }
  if (/\b(full name|display name|your name)\b/.test(combined)) {
    return withValue("full_name", values.fullName);
  }
  if (/\bname\b/.test(combined)) {
    return withValue(fieldPrefersHandle(field) ? "username" : "full_name", fieldPrefersHandle(field) ? values.accountHandle || values.username : values.fullName);
  }
  if (type === "number" && shouldFill) {
    return withValue("number", values.genericNumber);
  }
  if (asSelect && shouldFill) {
    return withValue("select", "");
  }
  if (textLike && shouldFill) {
    return withValue("generic_text", values.genericText);
  }
  return null;
}

async function collectVisibleAuthFormFields(page, options = {}) {
  if (!page || typeof page.locator !== "function") {
    return [];
  }
  const controls = page.locator("input, textarea, select");
  const maxFields = Math.min(parsePositiveInteger(options.maxFields, 80), 160);
  const fields = [];
  let count = 0;
  try {
  ...
  const limit = Math.min(count, maxFields);
  for (let index = 0; index < limit; index += 1) {
    ...
    try {
      const field = await locator.evaluate((element, fieldIndex) => {
        const read = (value, max = 240) =>
          typeof value === "string" ? value.trim().replace(/\s+/g, " ").slice(0, max) : "";
        const readText = (node, max = 240) => read(node?.textContent || "", max);
        ...
        if (id) {
          ...
          const text = readText(explicitLabel);
          if (text && !labels.includes(text)) {
            labels.push(text);
          }
        }
        const wrappedLabel = element.closest("label");
        const wrappedText = readText(wrappedLabel);
        if (wrappedText && !labels.includes(wrappedText)) {
          labels.push(wrappedText);
        }
        const ariaLabelledBy = read(element.getAttribute("aria-labelledby"), 512)
          .split(/\s+/)
          .map((labelId) => readText(document.getElementById(labelId)))
          .filter(Boolean)
          .join(" ");
        const nearbyText = (() => {
          const container = element.closest("[class*='field'],[class*='form'],[class*='input'],.field,.form-group,.input-group") || element.parentElement;
          if (!container || container === document.body) {
            return "";
          }
          return readText(container, 320);
        })();
        const tagName = element.tagName.toLowerCase();
        const type = read(element.getAttribute("type") || (tagName === "select" ? "select" : tagName), 64).toLowerCase();
        const options = tagName === "select"
          ? Array.from(element.options || []).map((option) => ({
              value: read(option.value, 256),
              text: read(option.textContent, 256),
        ...
        return {
          ...
          inputMode: read(element.getAttribute("inputmode"), 64),
          pattern: read(element.getAttribute("pattern"), 512),
          title: read(element.getAttribute("title")),
          ariaLabel: read(element.getAttribute("aria-label")),
          ariaLabelledBy,
          nearbyText,
          value: read(element.value, 512),
          required: element.required === true || read(element.getAttribute("aria-required")).toLowerCase() === "true",
      ...
      if (field && isPlainObject(field)) {
        fields.push(field);
      }
    } catch {
      // Ignore fields that disappear during inspection.
    }
  }
  return fields;
}

async function fillAuthFieldFromDecision(page, field, decision, options = {}) {
  if (!page || !field || !decision || decision.unsafe === true) {
    return false;
  }
  ...
  if (decision.mode === "check" || type === "checkbox") {
    try {
  ...
  if (tagName === "select" || decision.mode === "select") {
    const optionValue = selectOptionForAuthField(field, decision.value);
    if (!optionValue) {
      return false;
    }
    try {
  ...
  return fillBestEffort(locator, decision.value, { timeout });
}

async function fillSafeAuthFormFields(page, values = {}, options = {}) {
  if (!page || !isPlainObject(values)) {
    return { filled: [], skippedUnsafe: [], unfilledRequired: [] };
  }
  const fields = await collectVisibleAuthFormFields(page, options);
  const filled = [];
  const skippedUnsafe = [];
  ...
  for (const field of fields) {
    if (authFieldIsIgnorable(field)) {
      if (field.required === true) {
        unfilledRequired.push(authFieldSummary(field, { reason: "ignored" }));
      }
      continue;
    }
    if (authFieldHasValue(field)) {
    ...
    const decision = resolveAuthFieldFill(field, values, options);
    if (decision?.unsafe === true) {
      if (field.required === true) {
        skippedUnsafe.push(authFieldSummary(field, { reason: decision.reason || "unsafe", key: decision.key || null }));
      }
      continue;
    }
    if (!decision) {
      if (field.required === true) {
        unfilledRequired.push(authFieldSummary(field, { reason: "unclassified" }));
      }
      continue;
    }
    const ok = await fillAuthFieldFromDecision(page, field, decision, options);
    if (ok) {
      filled.push(authFieldSummary(field, { key: decision.key || "field" }));
    } else if (field.required === true) {
      unfilledRequired.push(authFieldSummary(field, { reason: "fill_failed", key: decision.key || null }));
    }
  }
  return { filled, skippedUnsafe, unfilledRequired };
}
...
async function retryInvalidAuthFields(page, failureSignals, locators, values = {}, options = {}) {
  ...
  const rawInvalidFields = Array.isArray(failureSignals?.invalidFields) ? failureSignals.invalidFields : [];
  const errorTexts = Array.isArray(failureSignals?.errorTexts) ? failureSignals.errorTexts : [];
  const invalidFields = rawInvalidFields.slice();
  const alreadyHasPhoneField = rawInvalidFields.some((field) => {
    const haystack = `${field?.name || ""} ${field?.label || ""} ${field?.type || ""}`.toLowerCase();
    return haystack.includes("phone") || haystack.includes("mobile") || haystack.includes("telephone");
  });
  if (
    !alreadyHasPhoneField &&
    errorTexts.some((text) => {
      const normalized = sanitizeString(text, 240).toLowerCase();
      return (
        (normalized.includes("phone") || normalized.includes("mobile") || normalized.includes("telephone")) &&
        /(required|missing|enter|provide|blank|empty)/.test(normalized)
      );
    })
  ) {
    invalidFields.push({
      ...
      name: "",
      type: "tel",
      validationMessage: "Phone field appears required."
    });
  }
...
async function performCredentialedLogin(page, runRequest, options = {}) {
  ...
  let password = sanitizeOptionalString(credentials?.password, 320) || null;
  if (!password && autoCreateAccount) {
    password = buildGeneratedPassword(runRequest, metadata);
  }
  ...
  const authFieldValues = buildGeneratedAuthFieldValues(runRequest, metadata, {
    fullName,
    firstName,
    lastName,
    phone,
  ...
  if (preferGoogle) {
    ...
    const genericFieldFill = await fillSafeAuthFormFields(page, authFieldValues, {
      signupMode: shouldFillNameFields,
      fillOptionalSafeFields: shouldFillNameFields,
      timeout: 6000
    });
    if (genericFieldFill.filled.length > 0) {
      pushRunLog(runLog, "auth_generic_fields_filled", {
        fields: genericFieldFill.filled.slice(0, 12)
      });
    }
    if (genericFieldFill.unfilledRequired.length > 0) {
      pushRunLog(runLog, "auth_required_fields_unfilled", {
        fields: genericFieldFill.unfilledRequired.slice(0, 12)
      });
    }
    if (genericFieldFill.skippedUnsafe.length > 0) {
      pushRunLog(runLog, "auth_required_fields_blocked", {
        fields: genericFieldFill.skippedUnsafe.slice(0, 12)
      });
      await captureCheckpoint(options, "auth-required-fields-blocked", page);
      throw new Error("Required sensitive auth fields were present and were not filled.");
    }
    ...
    if (captchaDetection.shouldResolve && typeof options.resolveCaptcha === "function") {
      ...
      if (captchaResult?.ok === true) {
        ...
        if (await authSurfaceStillVisible(page)) {
          ...
          const genericRestoredAfterCaptcha = await fillSafeAuthFormFields(page, authFieldValues, {
            signupMode: authModeBeforeSubmit === "signup",
            fillOptionalSafeFields: authModeBeforeSubmit === "signup",
            timeout: 6000
          });
          if (genericRestoredAfterCaptcha.filled.length > 0) {
            pushRunLog(runLog, "auth_generic_fields_restored_after_captcha", {
              fields: genericRestoredAfterCaptcha.filled.slice(0, 12)
            });
            await captureCheckpoint(options, "auth-generic-fields-restored-after-captcha", page);
          }
...
module.exports = {
  ...
  __private: {
    ...
    fieldPrefersHandle,
    collectVisibleAuthFormFields,
    fillSafeAuthFormFields,
    resolveAuthFieldFill,
    extractPasswordInstruction,
