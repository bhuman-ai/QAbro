...
function buildQueuePayload(options = {}) {
  ...
  return {
    ...
    report_markdown:
      sanitizeOptionalString(options.reportMarkdown, 200000) ||
      sanitizeOptionalString(existingPayload.report_markdown, 200000),
...
function getQueueStatusFromRow(row) {
  const payload = isPlainObject(row?.payload) ? row.payload : {};
  const queue = sanitizeQueue(payload.queue);
  return normalizeQueueLifecycleStatus(queue.status || row?.status);
}
...
async function estimateQueueInsight(row, options = {}) {
  ...
  if (!ACTIVE_QUEUE_STATUSES.has(currentStatus)) {
    return null;
  }
...
async function enqueueQaRun(runRequest, options = {}) {
  ...
  return {
    ...
    queue: summarizeQueueStatus(Array.isArray(data) && data[0] ? data[0] : row)
  };
}
...
async function getQaRunStatus(runId, options = {}) {
  ...
  const requiredOwnerUserId = sanitizeString(options.ownerUserId || options.owner_user_id, 128);
  if (requiredOwnerUserId) {
    const ownerUserId = extractOwnerUserId(loaded.row);
    if (!ownerUserId || ownerUserId !== requiredOwnerUserId) {
      return {
        ok: false,
        status: 404,
        error: "Run not found"
