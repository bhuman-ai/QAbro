...
function sanitizeQueue(value) {
  ...
  return {
    ...
    failure_stage: sanitizeOptionalString(queue.failure_stage, 128) || null,
    failure_code: sanitizeOptionalString(queue.failure_code, 128) || null
  };
}
...
function summarizeQueueStatus(row) {
  ...
  return {
    ...
    failure_stage: queue.failure_stage,
    failure_code: queue.failure_code,
...
function extractOwnerEmail(row) {
  const payload = isPlainObject(row?.payload) ? row.payload : {};
  const runRequest = isPlainObject(payload.run_request) ? payload.run_request : {};
  const metadata = isPlainObject(runRequest.metadata) ? runRequest.metadata : {};
  ...
  const candidates = [
    ...
    metadata.ownerEmail,
    reportMetadata.owner_email,
    reportMetadata.ownerEmail,
    payload.owner_email,
    payload.ownerEmail
  ];

  for (const candidate of candidates) {
    const value = sanitizeString(candidate, 320).toLowerCase();
    if (value) {
      return value;
    }
  }
...
function resolveQaReportReadAccess(row, options = {}) {
  ...
  const adminOk = authOk && options.adminOk === true;
  ...
  if (adminOk) {
    return { ok: true, status: 200, access_type: "admin" };
  }
  if (authOk) {
    if (!ownerUserId) {
...
function buildLegacyOwnerEmailPayloadFilter(ownerEmailFilter) {
  return `cs.${JSON.stringify({
    run_request: {
      metadata: {
        owner_email: ownerEmailFilter
      }
    }
  })}`;
}
...
function buildListQaReportsUrl(access, filters, fetchLimit, useComputedColumns = true) {
  ...
  const ownerEmailFilter = sanitizeString(filters.owner_email || filters.ownerEmail, 320).toLowerCase();
  ...
  if (ownerFilter) {
    ...
    listUrl.searchParams.set("payload", buildLegacyOwnerEmailPayloadFilter(ownerEmailFilter));
  }
  if (statusFilter) {
...
function rowMatchesFilters(row, filters) {
  ...
  const ownerUserId = extractOwnerUserId(row);
  const ownerEmail = extractOwnerEmail(row);
  ...
  if (ownerEmailFilter && String(ownerEmail || "").toLowerCase() !== ownerEmailFilter) {
    return false;
  }
...
async function listQaReports(filters = {}, options = {}) {
  ...
  if (response.ok && Array.isArray(rows) && !rows.length && ownerUserId && ownerEmail) {
    const emailFallbackFilters = {
      ...filters,
      owner_user_id: "",
      ownerUserId: "",
    ...
    const emailFallbackUrl = buildListQaReportsUrl(access, emailFallbackFilters, fetchLimit, true);
    response = await access.fetchImpl(emailFallbackUrl.toString(), {
      headers
    });
    ...
    if ((!response.ok || !Array.isArray(rows)) && isMissingReportComputedColumnsError(rows)) {
      const legacyEmailFallbackUrl = buildListQaReportsUrl(access, emailFallbackFilters, fetchLimit, false);
      response = await access.fetchImpl(legacyEmailFallbackUrl.toString(), {
        headers
      });
    ...
    if (response.ok && Array.isArray(rows)) {
      effectiveFilters = emailFallbackFilters;
    }
  }
  ...
  const filtered = (Array.isArray(rows) ? rows : []).filter((row) => rowMatchesFilters(row, effectiveFilters));
