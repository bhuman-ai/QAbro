...
const MCP_QA_RESOURCE_TEMPLATES = Object.freeze({
  ...
  manual_review_workflow: "qa://workflows/manual-review",
  manual_qa_live_evidence: "qa://manual/{session_id}/evidence.json",
...
function isTerminalStatus(statusPayload = {}) {
  ...
  return TERMINAL_REPORT_STATUSES.has(reportStatus);
}

function selectManualFeedbackPackage(session = {}, options = {}) {
  const agentFeedback = session && typeof session.agent_feedback === "object" ? session.agent_feedback : {};
  const rawPackages = Array.isArray(agentFeedback.packages)
    ? agentFeedback.packages
    : agentFeedback.latest
      ? [agentFeedback.latest]
      : [];
  const packages = rawPackages
    .filter((entry) => entry && typeof entry === "object" && sanitizeString(entry.markdown, 200000))
    .slice()
    .reverse();
  const requestedScope = sanitizeString(options.scope || "all", 24).toLowerCase();
  const scope = ["all", "item", "any"].includes(requestedScope) ? requestedScope : "all";
  const itemId = sanitizeString(options.item_id || options.itemId, 80);
  return (
    packages.find((entry) => {
      const entryScope = sanitizeString(entry.scope, 24).toLowerCase() || (entry.item_id ? "item" : "all");
      if (scope !== "any" && entryScope !== scope) return false;
      if (itemId && sanitizeString(entry.item_id, 80) !== itemId) return false;
      return true;
    }) || null
  );
}

function normalizeManualEvidenceKind(entry = {}) {
  const kind = sanitizeString(entry.kind || entry.type, 64).toLowerCase();
  ...
  if (kind === "screenshot" || kind === "image" || contentType.startsWith("image/")) return "image";
  return kind || "evidence";
}

function compareEvidenceCreatedAt(a = {}, b = {}) {
...
function summarizeManualDraftEvidence(session = {}, options = {}) {
  const checklist = Array.isArray(session?.checklist) ? session.checklist : [];
  const requestedItemId = sanitizeString(options.item_id || options.itemId, 80);
  const latestLimit = Math.max(1, Math.min(50, Number(options.latest_limit || options.latestLimit || 8)));
  const countsByKind = {};
  const evidence = [];
  let sequence = 0;

  checklist.forEach((item) => {
    const itemId = sanitizeString(item?.id, 80);
    if (requestedItemId && itemId !== requestedItemId) return;
    const media = Array.isArray(item?.evidence_media) ? item.evidence_media : [];
    const urls = Array.isArray(item?.evidence_urls) ? item.evidence_urls : [];
    const mediaUrls = new Set(
      media
        .map((entry) => sanitizeString(entry?.url || entry?.evidence_url || entry?.evidenceUrl, 4096))
        .filter(Boolean)
    );
    media.forEach((entry, index) => {
      const kind = normalizeManualEvidenceKind(entry);
      countsByKind[kind] = (countsByKind[kind] || 0) + 1;
      evidence.push({
        sequence: sequence++,
        item_id: itemId || null,
        item_title: sanitizeString(item?.title, 240) || null,
        index,
        kind,
        label: sanitizeString(entry?.label || entry?.filename || entry?.file_name, 240) || null,
        content_type: sanitizeString(entry?.content_type || entry?.contentType, 128) || null,
        byte_length: Number(entry?.byte_length || entry?.byteLength || 0) || null,
        url: sanitizeString(entry?.url || entry?.evidence_url || entry?.evidenceUrl, 4096) || null,
        created_at: sanitizeString(entry?.created_at || entry?.createdAt, 128) || null
      });
    });
    urls
      .filter((url) => {
        const safeUrl = sanitizeString(url, 4096);
        return safeUrl && !mediaUrls.has(safeUrl);
      })
      .forEach((url, index) => {
        countsByKind.link = (countsByKind.link || 0) + 1;
        evidence.push({
          sequence: sequence++,
          item_id: itemId || null,
          item_title: sanitizeString(item?.title, 240) || null,
          ...
          content_type: null,
          byte_length: null,
          url: sanitizeString(url, 4096) || null,
          created_at: null
        });
      });
  });

  const sortedEvidence = evidence.slice().sort(compareEvidenceCreatedAt);
  const latestEvidence = sortedEvidence.slice(-latestLimit).reverse();
  const totalCount = evidence.length;
  const videoSegments = evidence.filter((entry) => {
    const label = sanitizeString(entry.label, 240).toLowerCase();
    return entry.kind === "video" && label.includes("segment");
  }).length;

  return {
    ready: totalCount > 0,
    session_id: sanitizeString(session?.session_id || session?.sessionId, 128) || null,
    review_mode: sanitizeString(session?.review_mode || session?.reviewMode, 64) || null,
    total_count: totalCount,
    counts_by_kind: countsByKind,
    video_count: countsByKind.video || 0,
    video_segment_count: videoSegments,
    drawing_count: countsByKind.drawing || 0,
    image_count: countsByKind.image || 0,
    audio_count: countsByKind.audio || 0,
    link_count: countsByKind.link || 0,
    latest_evidence: latestEvidence.map(({ sequence: _sequence, ...entry }) => entry)
  };
}
...
function summarizeCodingAgentQaOutcome({ reportPayload = null, waitResult = null, share = null } = {}) {
  ...
  if (reportStatus === "failed_validation") {
    return {
      verdict: CODING_AGENT_QA_VERDICTS.NEEDS_REVIEW,
      pass: false,
      reason: "QA evidence or report validation failed, so no product pass/fail verdict is available.",
  ...
  if (reportStatus === "failed" || hasBlockingFinding) {
    return {
...
function buildQaResourceUri(kind, runId) {
  ...
  switch (kind) {
    ...
    case "manual_review_workflow":
      return "qa://workflows/manual-review";
    case "manual_qa_live_evidence":
      return `qa://manual/${encodeURIComponent(safeRunId)}/evidence.json`;
...
function createQaResourceReaders(apiClient) {
  ...
  return {
    ...
    async readManualQaLiveEvidence(sessionId, requestOptions = {}) {
      if (typeof apiClient.getManualQaSession !== "function") {
        throw new Error("Manual QA session reads are not available");
      }
      const payload = await apiClient.getManualQaSession(sessionId, requestOptions);
      const draftEvidence = payload.draft_evidence || summarizeManualDraftEvidence(payload.session, requestOptions);
      return {
        uri: buildQaResourceUri("manual_qa_live_evidence", sessionId),
        mimeType: "application/json",
        payload,
        text: JSON.stringify(
          {
...
function createQaApiClient(options = {}) {
  ...
  return {
    ...
    async requestHumanTest(input = {}, requestOptions = {}) {
      return request("api/human-test-requests", {
        method: "POST",
    ...
    async getHumanTestRequest(requestId, requestOptions = {}) {
      return request("api/human-test-requests", {
        method: "GET",
    ...
    async getManualQaSession(sessionId, requestOptions = {}) {
      const response = await request("api/manual-qa/sessions", {
        method: "GET",
      ...
      return {
        ...response,
        draft_evidence: summarizeManualDraftEvidence(response.session, requestOptions)
      };
    },
    async submitManualPreviewProposal(sessionId, proposal = {}, requestOptions = {}) {
      return request("api/manual-qa/preview-proposal", {
        method: "POST",
        body: {
    ...
    async submitManualPostFixReview(sessionId, review = {}, requestOptions = {}) {
      return request("api/manual-qa/post-fix-review", {
        method: "POST",
        body: {
          session_id: sessionId,
          review
    ...
    async getManualQaWorkPackets(sessionId, requestOptions = {}) {
      const response = await this.getManualQaSession(sessionId, requestOptions);
      return {
        ok: true,
        session_id: sanitizeString(response.session?.session_id || sessionId, 128),
        work_packets: Array.isArray(response.session?.work_packets) ? response.session.work_packets : [],
        session: response.session,
        status: response
      };
    },
    async waitForManualEvidence(sessionId, waitOptions = {}) {
      const timeoutSeconds = Math.max(1, Number(waitOptions.timeout_seconds || waitOptions.timeoutSeconds || DEFAULT_TIMEOUT_SECONDS));
      const pollIntervalSeconds = Math.max(0.1, Number(waitOptions.poll_interval_seconds || waitOptions.pollIntervalSeconds || DEFAULT_POLL_INTERVAL_SECONDS));
      const minimumEvidenceCount = Math.max(1, Number(waitOptions.minimum_evidence_count || waitOptions.minimumEvidenceCount || 1));
      const sinceEvidenceCount = Math.max(0, Number(waitOptions.since_evidence_count || waitOptions.sinceEvidenceCount || 0));
      const startedAt = Date.now();
      let pollCount = 0;
      let latest = null;
      let latestDraftEvidence = null;

      while (Date.now() - startedAt < timeoutSeconds * 1000) {
        if (waitOptions.signal?.aborted) {
          const abortError = new Error("Manual QA evidence wait aborted");
          abortError.name = "AbortError";
          throw abortError;
        }
        latest = await this.getManualQaSession(sessionId, waitOptions);
        pollCount += 1;
        latestDraftEvidence = latest.draft_evidence || summarizeManualDraftEvidence(latest.session, waitOptions);
        const totalCount = Number(latestDraftEvidence.total_count) || 0;
        const evidenceReady = totalCount >= minimumEvidenceCount && totalCount > sinceEvidenceCount;
        if (typeof waitOptions.onPoll === "function") {
          await waitOptions.onPoll(latest, {
            poll_count: pollCount,
            elapsed_ms: Date.now() - startedAt,
            ...
            video_segment_count: Number(latestDraftEvidence.video_segment_count) || 0
          });
        }
        if (evidenceReady) {
          return {
            ...
            poll_count: pollCount,
            elapsed_ms: Date.now() - startedAt,
            draft_evidence: latestDraftEvidence,
    ...
    async waitForManualFeedback(sessionId, waitOptions = {}) {
      const timeoutSeconds = Math.max(1, Number(waitOptions.timeout_seconds || waitOptions.timeoutSeconds || DEFAULT_TIMEOUT_SECONDS));
      const pollIntervalSeconds = Math.max(0.1, Number(waitOptions.poll_interval_seconds || waitOptions.pollIntervalSeconds || DEFAULT_POLL_INTERVAL_SECONDS));
      const waitForever = waitOptions.wait_forever === true || waitOptions.waitForever === true;
      const startedAt = Date.now();
      let pollCount = 0;
      let latest = null;

      while (waitForever || Date.now() - startedAt < timeoutSeconds * 1000) {
        if (waitOptions.signal?.aborted) {
          const abortError = new Error("Manual QA feedback wait aborted");
          abortError.name = "AbortError";
          throw abortError;
        }
        latest = await this.getManualQaSession(sessionId, waitOptions);
        pollCount += 1;
        const feedback = selectManualFeedbackPackage(latest.session, waitOptions);
        if (typeof waitOptions.onPoll === "function") {
          await waitOptions.onPoll(latest, {
            poll_count: pollCount,
            elapsed_ms: Date.now() - startedAt,
            feedback_ready: Boolean(feedback)
          });
        }
        if (feedback) {
          return {
