...
function buildCodingAgentQaInput(input = {}) {
  ...
  const mergedScenarios = [...generatedScenarios, ...baseScenarioList].slice(0, 12);
  const expectedSuccess =
    sanitizeString(input.expected_success || input.expectedSuccess, 1000) ||
    (acceptanceCriteria.length ? acceptanceCriteria.join(" | ") : "");
  const taskToTry =
    sanitizeString(input.task_to_try || input.taskToTry, 1000) ||
    (workSummary
  ...
  return {
    ...input,
    feature_name:
      sanitizeString(input.feature_name || input.featureName, 240) ||
