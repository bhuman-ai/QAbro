...
function selectEligibleTesterApplications(applications = [], activeRequests = [], assignmentType = "qualification") {
  ...
  const requiredStatus = assignmentType === "paid" ? "approved" : "applied";
  for (const application of applications) {
    ...
    const eligible =
      application?.status === requiredStatus &&
...
async function notifyEligibleTestersAboutJob(request = {}, options = {}) {
  ...
  const assignmentType = request.assignment_type === "paid" ? "paid" : "qualification";
  const requiredStatus = assignmentType === "paid" ? "approved" : "applied";

  const [applicationsResult, ...activeResults] = await Promise.all([
    listApplications({ status: requiredStatus, limit: 200 }, options),
  ...
  const eligible = selectEligibleTesterApplications(
          ...
          sendEmail(
            {
              ...
              assignmentType,
              testerPayCents: request.tester_pay_cents,
              testerPayCurrency: request.tester_pay_currency,
              jobsUrl
            },
            options
          )
