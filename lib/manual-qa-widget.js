...
function buildManualQaWidgetScript(config = {}) {
    ...
    state.recorder.onstop = async () => {
      ...
      try {
        ...
        if (!chunkRefs.length) {
          ...
          if (state.resolveRecordingSave) state.resolveRecordingSave(false);
          return;
        }
        await finishRecordingUpload(chunkRefs, contentType, filename);
        if (state.resolveRecordingSave) state.resolveRecordingSave(true);
      } catch (error) {
        toast(error.message || "Could not upload recording");
        if (state.rejectRecordingSave) state.rejectRecordingSave(error);
      } finally {
        state.chunkUploads = [];
  ...
  async function stopRecordingAndWait() {
    ...
    try {
      return (await savePromise) !== false;
    } catch {
      return false;
    }
  }
