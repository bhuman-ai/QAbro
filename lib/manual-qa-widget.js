...
function buildManualQaWidgetScript(config = {}) {
  ...
  root.innerHTML = \`
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .bud-capture-panel.is-open { display: block; }
      .bud-capture-title { color: var(--bud-text); font-size: 13px; font-weight: 900; line-height: 1.2; }
      ...
      @media (max-width: 860px) {
        .bud-capture-panel { left: 8px; right: 8px; top: auto; bottom: 76px; width: auto; }
        .bud-panel { bottom: 256px; }
      }
    ...
    <aside class="bud-capture-panel" aria-label="Capture review evidence">
      ...
      <div class="bud-tool-row" aria-label="Draw tools">
  ...
  function render() {
    ...
    items.forEach((item, itemIndex) => {
      ...
      selectButton.addEventListener("click", () => changeSelectedItem(item.id, { navigate: true }).catch((error) => toast(error.message || "Could not change item")));
