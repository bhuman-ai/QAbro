...
function buildManualQaWidgetScript(config = {}) {
  ...
  root.innerHTML = \`
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .bud-close svg, .bud-nav-btn svg, .bud-tool svg, .bud-mic svg, .bud-note-tool svg { width: 18px; height: 18px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
      ...
      .bud-status[data-status="pass"].is-active, .bud-status[data-status="pass"]:hover { border-color: rgba(84, 209, 138, .7); background: rgba(84, 209, 138, .16); color: #c8ffdc; }
      .bud-status[data-status="fail"].is-active, .bud-status[data-status="fail"]:hover { border-color: rgba(255, 107, 122, .72); background: rgba(255, 107, 122, .16); color: #ffd5da; }
      .bud-status[data-status="confusing"].is-active, .bud-status[data-status="confusing"]:hover { border-color: rgba(255, 209, 102, .72); background: rgba(255, 209, 102, .15); color: #fff1c4; }
      .bud-status[data-status="blocked"].is-active, .bud-status[data-status="blocked"]:hover { border-color: rgba(189, 198, 211, .6); background: rgba(255, 255, 255, .12); color: var(--bud-text); }
      .bud-note-wrap {
        padding: 12px 14px;
        border-bottom: 1px solid var(--bud-line);
      ...
      .bud-item-status { color: var(--bud-faint); font-size: 10px; font-weight: 900; text-transform: uppercase; }
      .bud-tray {
        position: fixed;
        right: 14px;
        bottom: 18px;
      ...
      @media (max-width: 620px) {
        ...
        .bud-tray { left: 8px; right: 8px; bottom: 10px; justify-content: center; border-radius: 18px; }
        .bud-status-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
      }
    </style>
    ...
    <section class="bud-panel" aria-label="BeforeUsersDo review">
      ...
      <div class="bud-body">
        <div class="bud-current">
          ...
          <div class="bud-status-grid" aria-label="Mark current item">
            <button class="bud-status" data-status="pass" type="button">Pass</button>
            <button class="bud-status" data-status="fail" type="button">Fail</button>
            <button class="bud-status" data-status="confusing" type="button">Confusing</button>
            <button class="bud-status" data-status="blocked" type="button">Blocked</button>
          </div>
        </div>
        <div class="bud-note-wrap">
          <div class="bud-note-head">
            <div>
              ...
              <div class="bud-note-hint" data-role="recording-state">Tap the mic to record screen and voice.</div>
            </div>
            <div class="bud-note-actions">
              <button class="bud-mic" data-action="record" type="button" aria-label="Start voice recording" title="Start voice recording">
                <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><path d="M12 19v3"></path><path d="M8 22h8"></path></svg>
              </button>
              <button class="bud-note-tool" data-action="toggle-tools" type="button" aria-label="More review tools" aria-expanded="false" title="More review tools">
    ...
    <div class="bud-tray" aria-label="Review tools">
      ...
      <button class="bud-tool" data-action="save-drawing" type="button" aria-label="Save drawing" title="Save drawing">
  ...
  function render() {
    ...
    progressEl.textContent = \`\${reviewed} / \${items.length} done\`;
    ...
    messageEl.textContent = current
      ? [current.instructions || "Mark this item after you test it.", current.expected ? "Expected: " + current.expected : ""].filter(Boolean).join(" ")
    ...
    root.querySelectorAll("[data-status]").forEach((button) => {
      button.classList.toggle("is-active", current && button.getAttribute("data-status") === current.status);
    });
    ...
    items.forEach((item, itemIndex) => {
      ...
      button.innerHTML = \`<span class="bud-item-index">\${itemIndex + 1}</span><span class="bud-item-title">\${escapeHtml(item.title || "Checklist item")}</span><span class="bud-item-status">\${escapeHtml(item.status || "pending")}</span>\`;
      button.addEventListener("click", () => setSelectedItem(item.id, { navigate: true }));
  ...
  function selectByOffset(offset) {
    ...
    setSelectedItem(items[nextIndex].id, { navigate: true });
  }
  ...
  function setRecordingUi() {
    ...
    recordButton.setAttribute("aria-label", state.recording ? "Stop voice recording" : "Start voice recording");
    recordButton.setAttribute("title", state.recording ? "Stop voice recording" : "Start voice recording");
    ...
    recordingStateEl.textContent = state.recording
      ...
      : "Tap the mic to record screen and voice.";
  }
  ...
  function setToolsUi() {
    ...
    toolsButton.setAttribute("title", state.toolsOpen ? "Hide review tools" : "More review tools");
  }
  ...
  root.querySelector("[data-action='prev']").addEventListener("click", () => selectByOffset(-1));
  root.querySelector("[data-action='next']").addEventListener("click", () => selectByOffset(1));
