...
function buildManualQaWidgetScript(config = {}) {
  ...
  function setCommentMode(active) {
    ...
    commentButton.setAttribute("title", state.commentMode ? "Cancel comment (C)" : "Add comment on page (C)");
  }
  ...
  function positionCommentBoxForDraft(draft) {
    const clientX = Number.isFinite(Number(draft?.client_x)) ? Number(draft.client_x) : Number(draft?.page_x || 0) - window.scrollX;
    const clientY = Number.isFinite(Number(draft?.client_y)) ? Number(draft.client_y) : Number(draft?.page_y || 0) - window.scrollY;
    positionCommentBox(clientX, clientY);
  }

  function openExistingComment(entry) {
    const context = targetContextFromPoint(
      Number(entry.page_x || 0) - window.scrollX,
      Number(entry.page_y || 0) - window.scrollY
    );
    state.commentDraft = {
      mode: "edit",
      ...
      page_x: Math.round(Number(entry.page_x || 0)),
      page_y: Math.round(Number(entry.page_y || 0)),
      client_x: Math.round(Number(entry.page_x || 0) - window.scrollX),
      client_y: Math.round(Number(entry.page_y || 0) - window.scrollY),
      ...
      target_selector: entry.target_selector || context.target_selector || "",
  ...
  function renderCommentPins(item) {
    ...
    comments.forEach((entry) => {
      ...
      pin.style.left = Math.round(entry.page_x) + "px";
      pin.style.top = Math.round(entry.page_y) + "px";
  ...
  function setDrawingActive(active) {
    ...
    drawButton.setAttribute("title", state.drawing ? "Stop drawing (D)" : "Draw on page (D)");
  }
