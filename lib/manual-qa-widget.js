...
function buildManualQaWidgetScript(config = {}) {
  ...
  root.innerHTML = \`
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .bud-pill {
        ...
        font-size: 14px;
        font-weight: 850;
        box-shadow: 0 14px 38px rgba(17, 24, 39, .24);
        pointer-events: auto;
        z-index: 3;
      }
      .bud-dot {
      ...
      .bud-progress { margin-top: 8px; color: #334155; font-size: 12px; font-weight: 850; }
      ...
      .bud-close svg, .bud-nav-btn svg, .bud-tool svg { width: 18px; height: 18px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
      ...
      .bud-muted { margin: 8px 0 0; color: #475569; font-size: 12px; line-height: 1.42; }
      .bud-nav {
        display: grid;
      ...
      .bud-count { color: #334155; font-size: 12px; font-weight: 850; text-align: center; }
      .bud-status-grid {
        display: grid;
      ...
      .bud-status[data-status="pass"].is-active, .bud-status[data-status="pass"]:hover { border-color: #10b981; background: #ecfdf5; color: #047857; }
      .bud-status[data-status="fail"].is-active, .bud-status[data-status="fail"]:hover { border-color: #ef4444; background: #fef2f2; color: #b91c1c; }
      .bud-status[data-status="confusing"].is-active, .bud-status[data-status="confusing"]:hover { border-color: #f59e0b; background: #fffbeb; color: #92400e; }
      .bud-status[data-status="blocked"].is-active, .bud-status[data-status="blocked"]:hover { border-color: #64748b; background: #f1f5f9; color: #334155; }
      .bud-note-wrap { padding: 12px 14px; border-bottom: 1px solid #e2e8f0; }
      .bud-note {
        width: 100%;
        min-height: 84px;
        resize: vertical;
      ...
      .bud-item.is-selected { border-color: #2563eb; background: #eff6ff; }
      .bud-item-index {
        width: 22px;
        height: 22px;
        display: inline-flex;
      ...
      .bud-item-status { color: #64748b; font-size: 10px; font-weight: 900; text-transform: uppercase; }
      .bud-tray {
        position: fixed;
        right: 14px;
        bottom: 18px;
        ...
        background: #111827;
        padding: 8px;
        box-shadow: 0 18px 52px rgba(17, 24, 39, .28);
        pointer-events: auto;
        z-index: 4;
      }
      ...
      .bud-tool:hover, .bud-tool.active { background: #2563eb; color: #fff; }
      .bud-tool.danger { background: #dc2626; color: #fff; }
      .bud-tool:disabled, .bud-nav-btn:disabled { opacity: .45; cursor: not-allowed; }
      .bud-divider { width: 1px; height: 26px; background: rgba(255, 255, 255, .18); }
      .bud-toast {
        position: fixed;
        left: 50%;
        bottom: 20px;
    ...
    <div class="bud-tray" aria-label="Review tools">
      <button class="bud-tool" data-action="record" type="button" aria-label="Start recording" title="Start recording">
        <svg viewBox="0 0 24 24" aria-hidden="true"><circle class="fill" cx="12" cy="12" r="6"></circle></svg>
      </button>
  ...
  function setRecordingUi() {
    ...
    recordButton.setAttribute("title", state.recording ? "Stop recording" : "Start recording");
  }
