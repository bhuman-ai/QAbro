...
function buildManualQaWidgetScript(config = {}) {
  ...
  const state = {
    ...
    recordingSavePromise: null,
    resolveRecordingSave: null,
    rejectRecordingSave: null,
    streams: [],
    recordingUrl: "",
    pageVisits: [],
    consoleEvents: [],
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .bud-capture-panel {
        position: fixed;
        top: 52px;
        right: calc(28px + var(--bud-panel-width) + 12px);
        bottom: auto;
        width: auto;
        display: none;
        flex-direction: column;
  ...
  function render() {
    ...
    subtitleEl.textContent = freestyle ? "Record anything you notice." : "Test each item on this page.";
    ...
    messageEl.textContent = freestyle
      ? "Record, draw, or type notes while you move through the product. Send all when done."
      : current
  ...
  function ensureCanvasReady() {
    const ratio = window.devicePixelRatio || 1;
    const expectedWidth = Math.max(1, Math.floor(Math.max(1, window.innerWidth) * ratio));
    const expectedHeight = Math.max(1, Math.floor(Math.max(1, window.innerHeight) * ratio));
  ...
  async function autoSaveDrawingIfNeeded(options = {}) {
    ...
    try {
      await uploadDataUrl("screenshot", dataUrl, "drawing-annotation.png", "image/png", {
        silent: true,
  ...
  async function finishRecordingUpload(chunkRefs, contentType, filename, partial = false) {
    const item = selectedItem();
    if (!item) {
      toast("Pick an item first.");
      return;
    ...
    const payload = await api("/api/manual-qa/widget-evidence-chunks", {
      ...
      body: JSON.stringify({
        ...
        item_id: item.id,
        kind: "video",
        label: partial ? "Video recording (partial)" : "Video recording",
        filename,
        content_type: contentType,
        chunks: chunkRefs
      })
    ...
    state.session = payload.session || state.session;
    toast(partial ? "Partial recording saved. Some later data was lost." : "Recording saved", partial ? 6000 : 2200);
    render();
  }
  ...
  async function startRecording() {
    ...
    state.recordingSavePromise = new Promise((resolve, reject) => {
      state.resolveRecordingSave = resolve;
      state.rejectRecordingSave = reject;
    });
    ...
    state.recorder.ondataavailable = (event) => {
      ...
      const upload = uploadRecordingChunk(event.data, chunkIndex, contentType, filename)
        .then((chunk) => ({ ok: true, index: chunkIndex, chunk }))
        .catch((error) => ({ ok: false, index: chunkIndex, error }));
      state.chunkUploads.push(upload);
    };
    ...
    state.recorder.onstop = async () => {
      ...
      setRecordingUi();
      try {
        toast("Saving recording...", 4000);
        await new Promise((resolve) => window.setTimeout(resolve, 0));
        const results = await Promise.all(state.chunkUploads);
        const failed = results.filter((result) => !result.ok);
        const firstFailedIndex = failed.length
          ? Math.min(...failed.map((result) => Number.isInteger(result.index) ? result.index : Number.MAX_SAFE_INTEGER))
          : Number.MAX_SAFE_INTEGER;
        const chunkRefs = results
          .filter((result) => result.ok && result.chunk && result.index < firstFailedIndex)
          .sort((a, b) => a.index - b.index)
          .map((result) => result.chunk);
        if (!chunkRefs.length) {
          const message = failed[0]?.error?.message || "No recording data was captured.";
          noteRecordingFailure(message);
          toast(message, 6000);
          if (state.resolveRecordingSave) state.resolveRecordingSave({ ok: false, error: message });
          return;
        }
        await finishRecordingUpload(chunkRefs, contentType, filename, failed.length > 0);
        if (failed.length) {
          noteRecordingFailure("Recording saved partially after an upload failure.");
        }
        if (state.resolveRecordingSave) state.resolveRecordingSave({ ok: true, partial: failed.length > 0 });
      } catch (error) {
        const message = error.message || "Could not upload recording";
        noteRecordingFailure(message);
        toast(message, 6000);
        if (state.resolveRecordingSave) state.resolveRecordingSave({ ok: false, error: message });
      } finally {
        ...
        state.recordingChunkIndex = 0;
        state.recordingSaving = false;
        state.recordingSavePromise = null;
        state.resolveRecordingSave = null;
        state.rejectRecordingSave = null;
        setRecordingUi();
      }
    };
  ...
  async function stopRecordingAndWait() {
    const savePromise = state.recordingSavePromise;
    if (state.recording) {
      toast("Stopping recording...", 4000);
      stopRecording();
    }
    if (!savePromise) return true;
    try {
      const result = await Promise.race([
        savePromise,
        new Promise((resolve) => window.setTimeout(() => resolve({ ok: false, timeout: true }), 45000))
      ]);
      if (result?.timeout) {
        toast("Recording is still saving. Keep this tab open and press Send again.", 8000);
        return false;
      }
      if (result?.ok === false) {
        toast("Recording was not saved. Other feedback will still be sent.", 7000);
        return true;
      }
      return result !== false;
    } catch (error) {
