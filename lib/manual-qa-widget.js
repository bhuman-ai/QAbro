...
function buildManualQaWidgetScript(config = {}) {
  ...
  function enqueueRecordingSegmentUpload(blob, itemId, contentType, segmentIndex) {
    ...
    const upload = blobToDataUrl(typedBlob)
      ...
      .then((payload) => ({ ok: true, index: segmentIndex, payload }))
      .catch((error) => {
