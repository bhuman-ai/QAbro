...
function normalizeHumanTestRequestPayload(input = {}, owner = {}, options = {}) {
  ...
  const creditAmountCents = Math.max(
    0,
    Math.round(Number(safeInput.qa_credit_amount_cents || safeInput.qaCreditAmountCents) || 0)
  );
  if (fundingType === "qa_credit" && creditAmountCents < 100) {
    return { ok: false, status: 400, error: "Choose at least $1 of QA credit for this request" };
  }
  ...
  return {
    ...
    payload: {
      ...
      assignment_type: fundingType === "qa_credit" ? "paid" : "qualification",
      tester_pay_cents: fundingType === "qa_credit" ? creditAmountCents : 0,
      tester_pay_currency: "USD",
      payout_status: fundingType === "qa_credit" ? "pending" : "not_applicable",
