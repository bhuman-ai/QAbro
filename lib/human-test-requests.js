...
function normalizeHumanTestRequestPayload(input = {}, owner = {}, options = {}) {
  ...
  return {
    ...
    payload: {
      ...
      duration_minutes: Math.max(10, Math.min(60, Number(safeInput.duration_minutes || safeInput.durationMinutes) || 30)),
...
function normalizeHumanTestRequestRow(value, options = {}) {
  ...
  const normalized = {
    ...
    duration_minutes: Math.max(10, Math.min(60, Number(row.duration_minutes) || 30)),
...
async function claimHumanTestRequest(id, tester = {}, options = {}) {
  ...
  const { createQaTrial } = require("./qa-trials");
...
async function assignHumanTestRequest(id, input = {}, options = {}) {
  const loaded = await getHumanTestRequest(id, {}, { ...options, includeCredentials: true });
  if (!loaded.ok) return loaded;
  if (loaded.request.status !== "queued") {
    return { ok: false, status: 409, error: "This human test request has already been assigned" };
  }
  if (loaded.request.credentials_error) {
    return { ok: false, status: 500, error: loaded.request.credentials_error };
  }
  ...
  const knownIssues = normalizeStringList(input.known_issues || input.knownIssues, 24, 1200);
  if (!isValidEmail(testerEmail)) return { ok: false, status: 400, error: "tester_email must be a valid email" };
  if (!knownIssues.length) return { ok: false, status: 400, error: "Add at least one private benchmark issue before assigning the tester" };

  const { createQaTrial } = require("./qa-trials");
  const created = await createQaTrial(
    {
      product_name: loaded.request.product_name,
      target_url: loaded.request.target_url,
      lead_email: loaded.request.owner_email,
      tester_name: testerName,
      tester_email: testerEmail,
      test_focus: loaded.request.test_focus,
      known_issues: knownIssues,
      duration_minutes: loaded.request.duration_minutes,
      lead_preapproved: true,
      source_request_id: loaded.request.id,
      access_mode: loaded.request.access_mode,
      access_details: loaded.request.access,
      credentials: loaded.request.credentials,
      assignment_type: loaded.request.assignment_type,
      tester_pay_cents: loaded.request.tester_pay_cents,
      tester_pay_currency: loaded.request.tester_pay_currency,
      payout_status: loaded.request.payout_status
    },
    {
      ...options,
      ownerUserId: loaded.request.owner_user_id,
      ownerEmail: loaded.request.owner_email,
      launchedBy: "human_test_request_assignment"
    }
  );
  ...
  const updated = await patchHumanTestRequest(
    ...
    {
      status: "assigned",
      assigned_tester_application_id: sanitizeOptionalString(input.tester_application_id || input.testerApplicationId, 128) || null,
      assigned_tester_name: testerName || null,
      assigned_tester_email: testerEmail,
      trial_session_id: created.session_id
    },
  ...
  if (!updated.ok) {
    return { ...created, warning: updated.error, request: loaded.request };
  }
  return { ...created, request: updated.request };
}
...
async function markHumanTestRequestPaid(id, options = {}) {
  ...
  if (loaded.request.status !== "completed" || loaded.request.payout_status !== "approved") {
