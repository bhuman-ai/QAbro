...
async function publishHumanTestRequest(id, input = {}, options = {}) {
  ...
  const newlyPublished = loaded.request.status !== "available";
  const published = await patchHumanTestRequest(
    loaded.request.id,
    {
      status: "available",
      private_benchmark: privateBenchmark,
      published_at: newlyPublished ? new Date().toISOString() : loaded.request.published_at || new Date().toISOString(),
      claimed_at: null,
...
async function claimHumanTestRequest(id, tester = {}, options = {}) {
  ...
  const applicationUpdate = await updateTesterApplication(
    {
