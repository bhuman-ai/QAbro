...
function normalizeHumanTestRequestPayload(input = {}, owner = {}, options = {}) {
  ...
  const testFocus = `${baseTestFocus} Also call out prominent wording that feels vague, generic, robotic, repetitive, over-explained, unsupported, or immediately machine-written. Quote the exact words and explain your honest reaction; do not guess who or what wrote them. Call out screens that immediately feel cluttered, generic, AI-templated, visually unbalanced, hard to scan, inconsistent, or unfinished. Point to the exact visual pattern and explain how it affects your trust or ability to continue; do not report personal taste as a problem.`;
...
async function publishHumanTestRequest(id, input = {}, options = {}) {
  ...
  if (firstPublication) {
    ...
    if (!access.ok) return access;
    const requestUrl = new URL(`${access.supabaseUrl}/rest/v1/${TABLE}`);
    requestUrl.searchParams.set("id", `eq.${loaded.request.id}`);
    requestUrl.searchParams.set("status", "eq.queued");
    requestUrl.searchParams.set("select", PUBLIC_COLUMNS);
    const response = await access.fetchImpl(requestUrl.toString(), {
      method: "PATCH",
      headers: buildSupabaseHeaders(access.serviceKey),
      body: JSON.stringify(publicationPatch)
    });
    const data = await readJsonResponse(response);
    if (!response.ok || !Array.isArray(data)) {
      return buildSupabaseError(response, data, "Could not publish the human test request");
    }
    if (!data[0]) {
      const concurrent = await getHumanTestRequest(loaded.request.id, {}, options);
      if (!concurrent.ok) return concurrent;
      if (concurrent.request.status !== "available") {
        return { ok: false, status: 409, error: "This human test request changed before it could be published" };
      }
      return {
        ok: true,
        status: 200,
    ...
    published = { ok: true, status: 200, request: normalizeHumanTestRequestRow(data[0]) };
  } else {
    published = await patchHumanTestRequest(loaded.request.id, publicationPatch, options);
  }
