...
const DEFAULT_RECORDING_AGGREGATOR_MODEL = "openai/gpt-4.1-nano";
...
function migrateRecordingAggregatorModel(model) {
  const normalized = sanitizeString(model, 256);
  if (["gpt-4.1-mini", "openai/gpt-4.1-mini"].includes(normalized.toLowerCase())) {
    return DEFAULT_RECORDING_AGGREGATOR_MODEL;
  }
  return normalized || DEFAULT_RECORDING_AGGREGATOR_MODEL;
}
...
function resolveManualQaRecordingAnalyzerConfig(options = {}) {
  ...
  const aggregatorModel = migrateRecordingAggregatorModel(
