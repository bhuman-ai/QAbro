...
function isRetryableTwoCaptchaError(error) {
  const text = sanitizeOptionalString(error, 512) || "";
  if (!text) {
    return false;
  }
  return (
    /ERROR_CAPTCHA_UNSOLVABLE/i.test(text) ||
    /ERROR_NO_SLOT_AVAILABLE/i.test(text) ||
    /timed out/i.test(text) ||
    /temporar/i.test(text)
  );
}

async function resetCaptchaChallenge(page, challenge) {
  if (!page || typeof page.evaluate !== "function") {
    return { ok: false, resetCount: 0, error: "Page handle is not available." };
  }

  try {
    const result = await page.evaluate(({ challenge: widget }) => {
      const textFrom = (value) => (typeof value === "string" ? value.trim() : "");
      let resetCount = 0;
      const type = textFrom(widget?.type).toLowerCase();
      ...
      const clearNamedFields = (names) => {
        for (const name of names) {
          for (const selector of [`textarea[name='${name}']`, `input[name='${name}']`]) {
      ...
      if (type === "recaptcha") {
        clearNamedFields(["g-recaptcha-response"]);
        if (safeCall(window.grecaptcha?.reset)) {
          resetCount += 1;
        }
        if (safeCall(window.grecaptcha?.enterprise?.reset)) {
          resetCount += 1;
        }
      } else if (type === "hcaptcha") {
        clearNamedFields(["h-captcha-response", "g-recaptcha-response"]);
        if (safeCall(window.hcaptcha?.reset)) {
          resetCount += 1;
        }
      } else if (type === "turnstile") {
        clearNamedFields(["cf-turnstile-response"]);
        if (safeCall(window.turnstile?.reset)) {
          resetCount += 1;
        }
      }
      ...
      return {
        ok: resetCount > 0,
        resetCount,
        error: resetCount > 0 ? null : "No supported captcha reset handler was available on the page."
      };
    }, { challenge });

    return isPlainObject(result)
      ? result
      : { ok: false, resetCount: 0, error: "Captcha reset returned no result." };
  } catch (error) {
    return {
      ok: false,
      resetCount: 0,
      error: error?.message || "Captcha reset failed."
    };
  }
}
...
async function attemptTwoCaptchaSolve(page, runtime, runLog, options = {}) {
  const maxAttempts = Math.max(
    1,
    Math.min(4, Number(runtime?.twoCaptcha?.maxAttempts) || 2)
  );
  const retryBackoffMs = Math.max(
    0,
    Math.min(30000, Number(runtime?.twoCaptcha?.retryBackoffMs) || 2000)
  );
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (!tokenResult.ok) {
      const retryable =
        ...
        attempt < maxAttempts &&
        isRetryableTwoCaptchaError(tokenResult.error);
      ...
      const resetResult = await resetCaptchaChallenge(page, challenge);
      appendRunLog(runLog, "twocaptcha_retry_scheduled", {
        type: challenge.type,
        attempt,
        next_attempt: attempt + 1,
        reset_ok: resetResult.ok === true,
        reset_count: resetResult.resetCount || 0,
        reason: tokenResult.error || "2Captcha solve failed"
      });
      if (retryBackoffMs > 0 && typeof page.waitForTimeout === "function") {
    ...
    if (!resolved && attempt < maxAttempts && challenge.type === "recaptcha" && challenge.version !== "v3") {
      const resetResult = await resetCaptchaChallenge(page, challenge);
      appendRunLog(runLog, "twocaptcha_retry_scheduled", {
        type: challenge.type,
        attempt,
        ...
        reset_ok: resetResult.ok === true,
        reset_count: resetResult.resetCount || 0,
        reason: "captcha_still_visible_after_token_apply"
      });
      if (retryBackoffMs > 0 && typeof page.waitForTimeout === "function") {
        await page.waitForTimeout(retryBackoffMs);
      }
  ...
  return lastFailure || {
    ok: false,
    skipped: false,
    error: "2Captcha solve attempts were exhausted."
  };
}
...
function buildSelfHostedRuntimeConfig(jobRequest, options = {}) {
  ...
  const twoCaptchaMaxAttempts = Math.max(
    1,
    Math.min(
      4,
      Number(
        options.twoCaptchaMaxAttempts ??
          metadata.twocaptcha_max_attempts ??
          process.env.SUBMISSION_SELF_HOSTED_2CAPTCHA_MAX_ATTEMPTS ??
          process.env.SUBMISSION_DO_2CAPTCHA_MAX_ATTEMPTS ??
          process.env.TWOCAPTCHA_MAX_ATTEMPTS
      ) || 2
    )
  );
  const twoCaptchaRetryBackoffMs = Math.max(
    0,
    Math.min(
      30000,
      Number(
        options.twoCaptchaRetryBackoffMs ??
          metadata.twocaptcha_retry_backoff_ms ??
          process.env.SUBMISSION_SELF_HOSTED_2CAPTCHA_RETRY_BACKOFF_MS ??
          process.env.SUBMISSION_DO_2CAPTCHA_RETRY_BACKOFF_MS ??
          process.env.TWOCAPTCHA_RETRY_BACKOFF_MS
      ) || 2000
    )
  );
  ...
  return {
    ...
    twoCaptcha: {
      ...
      maxAttempts: twoCaptchaMaxAttempts,
      retryBackoffMs: twoCaptchaRetryBackoffMs,
...
function getBrowserLaunchOptions(runtime) {
  ...
  if (channel && !launchOptions.executablePath && !["default", "none", "false", "0"].includes(channel.toLowerCase())) {
