...
function sanitizeAssociatedRepoFullNames(value, maxItems = 6) {
  return sanitizeStringList(value, maxItems, 320);
}

function normalizeBrandRepoConnectionRow(row, options = {}) {
  const safeRow = isPlainObject(row) ? row : {};
  const connectionMeta = sanitizeConnectionMetadata(safeRow.connection);
  const associatedRepoFullNames = sanitizeAssociatedRepoFullNames(connectionMeta.associated_repo_full_names, 8);
  const primaryRepoFullName = sanitizeOptionalString(safeRow.selected_repo_full_name, 320) || null;
  const normalizedAssociatedRepoFullNames = Array.from(
    new Set(
      [primaryRepoFullName, ...associatedRepoFullNames]
  ...
  return {
    ...
    selected_repo_full_name: primaryRepoFullName,
    associated_repo_full_names: normalizedAssociatedRepoFullNames,
...
function buildBrandRepoConnectionPayload(input, owner = {}) {
  ...
  const connectionMetadata = sanitizeConnectionMetadata(input?.connection);
  const associatedRepoFullNames =
    input?.associated_repo_full_names !== undefined || input?.associatedRepoFullNames !== undefined
      ? sanitizeAssociatedRepoFullNames(input?.associated_repo_full_names || input?.associatedRepoFullNames, 8)
      : null;
  if (associatedRepoFullNames) {
    connectionMetadata.associated_repo_full_names = associatedRepoFullNames;
  }
  if (input?.connection !== undefined || associatedRepoFullNames !== null) {
    payload.connection = connectionMetadata;
  }
