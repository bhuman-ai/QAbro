...
async function acceptQaTrialRecordingAnalysis(sessionId, token, options = {}) {
  const verified = await verifyQaTrialAccess(sessionId, token, options);
  if (!verified.ok) return verified;
  if (verified.role !== "tester") {
    return { ok: false, status: 403, error: "Only the tester can approve recording analysis" };
  }
  ...
  if (!hasManualQaRecordingAnalysisConsent(verified.trial)) {
    const now = new Date().toISOString();
    const trial = {
      ...verified.trial,
      tester: {
        ...verified.trial.tester,
        recording_analysis_consent_version: MANUAL_QA_RECORDING_ANALYSIS_CONSENT_VERSION,
        recording_analysis_consent_at: now
      },
      updated_at: now
    };
    updated = await updateManualQaQualificationTrial(sessionId, trial, {
      ...options,
      widgetAccessOk: true
    });
    if (!updated.ok) return updated;
  ...
  const finalSession = queued?.ok ? queued.session : updated.session;
  const finalRow = queued?.ok ? queued.row : updated.row;
  return {
    ok: true,
    status: queued?.ok ? 200 : 202,
    queue_pending: !queued?.ok,
    error_code: queued?.ok ? null : "recording_analysis_queue_pending",
    role: verified.role,
    view: buildPublicTrialView(finalSession, rawTrialFromRow(finalRow), verified.role, options)
  };
}
