...
function buildPublicTrialView(session, trial, role, options = {}) {
  ...
  const storedTesterName = sanitizeOptionalString(trial?.tester?.name, 180) || "Your tester";
  const testerPublicName = normalizeTesterPublicName(trial?.tester?.public_name) || "Your tester";
  const assignment = {
    type: assignmentType,
    tester_reward_type:
      sanitizeString(rawAssignment.tester_reward_type, 40).toLowerCase() === "qa_credit"
        ? "qa_credit"
        : "cash",
  ...
  return {
    ...
    tester: role === "lead"
      ? { public_name: testerPublicName === "Your tester" ? null : testerPublicName }
      : { name: storedTesterName },
    lead: {
    ...
    lead_rating: role === "tester"
      ? { score: null, note: null, rated_at: null }
      : {
...
function buildAdminTrialView(session, trial, options = {}) {
  ...
  return {
    ...
    tester: {
      ...
      public_name: normalizeTesterPublicName(trial?.tester?.public_name),
...
async function createQaTrial(input = {}, options = {}) {
  ...
  const rawTesterPublicName = sanitizeOptionalString(
    input.tester_public_name || input.testerPublicName,
    120
  );
  const testerPublicName = normalizeTesterPublicName(rawTesterPublicName);
  ...
  if (rawTesterPublicName && !testerPublicName) {
  ...
  const trial = {
    ...
    assignment: {
      ...
      tester_reward_type:
        sanitizeString(input.tester_reward_type || input.testerRewardType, 40).toLowerCase() === "qa_credit"
          ? "qa_credit"
          : "cash",
...
async function rateQaTrial(sessionId, token, input = {}, options = {}) {
  ...
  const updated = await updateManualQaQualificationTrial(sessionId, nextTrial, {
    ...
    expectedLeadRatingScore: verified.trial.lead_rating?.score ?? null,
    expectedLeadRatingNote: verified.trial.lead_rating?.note ?? null,
    expectedLeadRatingRatedAt: verified.trial.lead_rating?.rated_at ?? null
  });
  if (!updated.ok) return updated;
...
async function setQaTrialTesterPublicName(sessionId, input = {}, options = {}) {
  if (options.allowTesterPublicNameUpdate !== true) {
    return { ok: false, status: 403, error: "Tester public-name access required" };
  }
  const publicName = normalizeTesterPublicName(input.public_name || input.publicName);
  if (!publicName) {
    return { ok: false, status: 400, error: "public_name must be one first name" };
  }
  const loaded = await getManualQaSession(sessionId, options);
  if (!loaded.ok) return loaded;
  const trial = rawTrialFromRow(loaded.row);
  ...
  const updated = await updateManualQaQualificationTrial(sessionId, nextTrial, {
    ...options,
    testerPublicNameOnlyMutation: true,
    expectedTesterPublicName: trial.tester?.public_name ?? null
  });
  if (!updated.ok) return updated;
  return {
    ok: true,
    status: 200,
    trial: buildAdminTrialView(updated.session, rawTrialFromRow(updated.row), options)
  };
}
  ...
  return {
    ...
    items: listed.items
      ...
      .map((session) => ({
        ...
        tester_reward_type:
          sanitizeString(session.qualification_trial.assignment?.tester_reward_type, 40).toLowerCase() === "qa_credit"
            ? "qa_credit"
            : "cash",
