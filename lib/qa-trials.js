...
async function syncHumanTestRequestStatus(trial, status, options = {}) {
  ...
  try {
    ...
    return await patchHumanTestRequest(requestId, { status }, options);
  } catch (caught) {
    return {
      ok: false,
...
async function scoreQaTrial(sessionId, input = {}, options = {}) {
  ...
  if (!updated.ok) return updated;
  await syncHumanTestRequestStatus(nextTrial, "completed", options);
